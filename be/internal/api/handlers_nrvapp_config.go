...
func (s *Server) buildConfigEditor(w http.ResponseWriter, projectID string) (*configeditor.Service, bool) {
	...
	r := repo.NewNrvappConfigVersionRepo(s.pool.DB, s.clock)
...
func (s *Server) handleListNrvappConfigFiles(w http.ResponseWriter, r *http.Request) {
...
func (s *Server) handleGetNrvappConfigFile(w http.ResponseWriter, r *http.Request) {
	projectID := getProjectID(r)
	if projectID == "" {
		writeError(w, http.StatusBadRequest, "X-Project header required")
		return
	...
	ver, _ := repo.NewNrvappConfigVersionRepo(s.pool.DB, s.clock).LatestVersion(projectID, file)
...
func (s *Server) handlePutNrvappConfigFile(w http.ResponseWriter, r *http.Request) {
	projectID := getProjectID(r)
	if projectID == "" {
		writeError(w, http.StatusBadRequest, "X-Project header required")
		return
	...
	s.wsHub.Broadcast(&ws.Event{
		Type:      ws.EventNrvappConfigUpdated,
		ProjectID: projectID,
...
func (s *Server) handleGetNrvappConfigHistory(w http.ResponseWriter, r *http.Request) {
	projectID := getProjectID(r)
	if projectID == "" {
		writeError(w, http.StatusBadRequest, "X-Project header required")
		return
...
func (s *Server) handleRollbackNrvappConfig(w http.ResponseWriter, r *http.Request) {
	projectID := getProjectID(r)
	if projectID == "" {
		writeError(w, http.StatusBadRequest, "X-Project header required")
		return
