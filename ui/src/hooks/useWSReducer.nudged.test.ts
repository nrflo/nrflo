...
describe('useWSReducer — agent.nudged handler', () => {
  let qc: QueryClient
  ...
  function makeEvent(overrides: Partial<WSEventV2> = {}): WSEventV2 {
    return {
      type: 'agent.nudged' as WSEventType,
      project_id: 'proj1',
      ticket_id: 'tick1',
  ...
  describe('event type registration', () => {
    it('agent.nudged is a recognised WSEventType (TypeScript compile check)', () => {
      const eventType: WSEventType = 'agent.nudged'
      expect(eventType).toBe('agent.nudged')
    })

    it('agent.nudged event is handled (returns true, not ignored)', () => {
      const handled = dispatchV2Event(makeEvent({ sequence: 50 }), qc)
      expect(handled).toBe(true)
    })

    it('duplicate agent.nudged events are deduplicated', () => {
      const spy = vi.spyOn(qc, 'invalidateQueries')
    ...
    it('handles later attempt nudge event correctly', () => {
      ...
      const event = makeEvent({
        sequence: 5,
        data: { session_id: 'sess-xyz', agent_type: 'qa-verifier', attempt: 5, max: 5 },
      })
