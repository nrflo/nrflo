...
func TestHandleListDefaultTemplates_SixReadonly(t *testing.T) {
	...
	if len(list) != 20 {
		t.Fatalf("len = %d, want 20 (pre-filled readonly templates)", len(list))
	}
	...
	wantOrder := []string{"api-system-prompt", "callback", "crash-resume", "delegation-guidance", "doc-updater", "finish-reminder", "implementor", "low-context", "qa-verifier", "setup-analyzer", "system-prompt", "system-prompt-suffix", "test-writer", "ticket-creator", "tier-t0-bare", "tier-t0-decider", "tier-t1-executor", "tier-t2-extractor", "user-instructions", "working-set"}
...
func TestHandleListDefaultTemplates_IncludesUserCreated(t *testing.T) {
	...
	list := decodeDefaultTemplateList(t, rr)
	if len(list) != 21 {
		t.Errorf("len = %d, want 21 (20 readonly + 1 user-created)", len(list))
	}
}
