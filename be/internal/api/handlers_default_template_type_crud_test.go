...
// Split from handlers_default_template_type_test.go to stay under the
// 300-line cap (Rule 5): create/update/restore/delete type handling + JSON
// field presence.
...
func TestHandleGetDefaultTemplate_InjectableTypeInJSON(t *testing.T) {
	s := newDefaultTemplateServer(t)
	req := httptest.NewRequest(http.MethodGet, "/api/v1/default-templates/callback", nil)
	req.SetPathValue("id", "callback")
	rr := httptest.NewRecorder()
	s.handleGetDefaultTemplate(rr, req)
...
func TestHandleListDefaultTemplates_FilterAfterCRUD(t *testing.T) {
	...
	if len(list) != 20 {
		t.Errorf("injectable count = %d, want 20 (19 seeded + 1 created)", len(list))
	}
