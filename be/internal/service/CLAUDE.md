...
| `plan.go` (+ `plan_manifest`/`plan_validate[_refs]`/`plan_schema`/`plan_templates`/`plan_limits`.go) | Plan lifecycle: `PlanService` (draft/revise/approve/cancel/TTL-sweep), manifest v1 types+hash, `ValidatePlanManifest` (one aggregated error), template-library resolution + enabled-model recheck, `plan_max_*`/`plan_draft_ttl_min` caps. Reserved key `_workflow_plan` is server-owned, resolved ahead of `finding_schemas` — see `findings_emit.go` |
...
`EnsureGlobalDeepResearch(pool, clk, rootPath)` (`deep_research_seed.go` + `_data.go`) idempotently ensures the `__global__` project exists **and** has a `root_path` (backfilled even on installs predating it being runnable), then create-if-absent seeds the bundled `deep-research` workflow (`is_global=1`, `callable_as_subworkflow=1`, result key `report`; 6 `cli_interactive` agents — 5 claude + 1 codex GPT-5.6 `verify_b` — finding schemas, L2 `quorum:2`) via direct SQL at startup (`cli/serve.go` passes `<data>/global`), bypassing the service layer's model validation since it is shipped data. The agents run cli_interactive so the claude/codex CLIs self-authenticate (no server-side API credential needed). L2 verifiers are lean and lens-differentiated: verify_a (`opus_4_8_1m`, artifact reads of the cached originals, quote support), verify_b (codex, bounded `web_fetch` of new sources only, independent corroboration), verify_c (lean sonnet, source quality). Caller-context grounding is opt-in via the `mcp-external` `deep_research` `context` arg → run `external_context` → scope reads `${EXTERNAL_CONTEXT}`. Mutating `__global__` defs is admin-only — HTTP `api.denyNonAdminGlobalWrite`, socket `socket.denyGlobalWorkflowMutation`.
