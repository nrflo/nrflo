...
1. Create `nrworkflow/internal/db/migrations/NNNNNN_description.up.sql` and `.down.sql` (next sequence number, e.g. `000003_add_labels.up.sql`)
...
1. Update CLI command in `nrworkflow/internal/cli/agent.go` or `findings.go`
...
4. **Documentation updates:**
   - This file (`nrworkflow/CLAUDE.md`) - update HTTP API Endpoints section
...
All integration tests use `NewTestEnv(t)` from `nrworkflow/internal/integration/testenv.go`. It creates an isolated stack with fresh DB, socket server, WS hub, and seeded project + workflow definition.
