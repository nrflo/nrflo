...
func setupConfigVersionDB(t *testing.T) (*ConfigVersionRepo, string) {
	t.Helper()
	...
	return NewConfigVersionRepo(database.DB, clk), "proj-1"
}

func TestConfigVersionRepo_InsertAutoVersion(t *testing.T) {
	t.Parallel()
	r, projectID := setupConfigVersionDB(t)
...
func TestConfigVersionRepo_VersionsIndependentPerFile(t *testing.T) {
	t.Parallel()
	r, projectID := setupConfigVersionDB(t)

	// Two inserts for config.yaml → versions 1 and 2
	for i := 1; i <= 2; i++ {
		v := &model.ConfigVersion{ProjectID: projectID, File: "config.yaml", Content: []byte("v")}
		if err := r.Insert(v); err != nil {
	...
	v2 := &model.ConfigVersion{ProjectID: projectID, File: "other.yaml", Content: []byte("v")}
...
func TestConfigVersionRepo_LatestVersionUnknown(t *testing.T) {
	t.Parallel()
	r, projectID := setupConfigVersionDB(t)
...
func TestConfigVersionRepo_LatestVersionAfterInserts(t *testing.T) {
	t.Parallel()
	r, projectID := setupConfigVersionDB(t)

	for i := 0; i < 3; i++ {
		v := &model.ConfigVersion{ProjectID: projectID, File: "f.yaml", Content: []byte("c")}
		r.Insert(v)
	}
...
func TestConfigVersionRepo_GetSpecificVersion(t *testing.T) {
	t.Parallel()
	r, projectID := setupConfigVersionDB(t)
...
func TestConfigVersionRepo_GetNotFound(t *testing.T) {
	t.Parallel()
	r, projectID := setupConfigVersionDB(t)
...
func TestConfigVersionRepo_History_DescOrder(t *testing.T) {
	t.Parallel()
	r, projectID := setupConfigVersionDB(t)

	for _, content := range []string{"v1", "v2", "v3"} {
		v := &model.ConfigVersion{ProjectID: projectID, File: "h.yaml", Content: []byte(content)}
		if err := r.Insert(v); err != nil {
...
func TestConfigVersionRepo_HistoryEmpty(t *testing.T) {
	t.Parallel()
	r, projectID := setupConfigVersionDB(t)
...
func TestConfigVersionRepo_SequentialInsertsMonotonic(t *testing.T) {
	t.Parallel()
	r, projectID := setupConfigVersionDB(t)

	v1 := &model.ConfigVersion{ProjectID: projectID, File: "seq.yaml", Content: []byte("first")}
	...
	v2 := &model.ConfigVersion{ProjectID: projectID, File: "seq.yaml", Content: []byte("second")}
	if err := r.Insert(v2); err != nil {
