...
func (s *Spawner) prepareAPIViaCLISpawn(
	...
	specs, handlers, toolEnv, regErr := s.buildAPIRegistry(req, wfiID, agentDef, proc, "", true, false, false, model.DispatchSourceMCP)
	if regErr != nil {
	...
	// --model, not from proc.maxContext: bare "claude-opus-5" opens 200k, the
	...
	// CLI 1M) stays bare; opus-4-6/4-7/4-8/5 (API 1M > CLI 200k) gets the suffix.
