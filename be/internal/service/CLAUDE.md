...
`EnsureGlobalDeepResearch(pool, clk, rootPath)` (`deep_research_seed.go` + `_data.go`) idempotently ensures the `__global__` project exists with a `root_path`, then create-if-absent seeds the bundled `deep-research` workflow (`is_global=1`, `callable_as_subworkflow=1`, result key `report`; 6 `cli_interactive` agents, finding schemas, L2 `quorum:2`) via direct SQL at startup — bypassing service-layer model validation since it's shipped data; see the file pair for the agent roster/lens details. Caller-context grounding is opt-in via the `mcp-external` `deep_research` `context` arg → `${EXTERNAL_CONTEXT}`. Mutating `__global__` defs is admin-only — HTTP `api.denyNonAdminGlobalWrite`, socket `socket.denyGlobalWorkflowMutation`.
