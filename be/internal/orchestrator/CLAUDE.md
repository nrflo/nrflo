...
Every session-resolving path — `ListTools`/`CallTool` (`orchestrator_tools.go`), the four control paths sharing `spawnerForSession` (`orchestrator_control.go`), and `SignalSessionReady` (`orchestrator_take_control.go`, which broadcasts over both maps rather than resolving one) — reaches `auxSpawners` as well as the run's own `runState.spawners`. The planner needs the second hop in both directions, since it is `cli_interactive` (tools arrive over the socket bridge, heartbeats via `record-event`) and a `revise_plan` re-run happens after `runLoop`'s defer already deleted the `runState`. Register a one-off child there via `OnSessionRegister`/`OnSessionUnregister`, never by synthesizing a `runState` — `IsRunning`/`StopWorkflow` act on that map. Missing the registration is silent: tools/list returns `[]`, every hook bump is a no-op, and SessionStart never reaches the child's prompt-delivery wait — each surfaces as an agent stalling at `stall_start_timeout_sec` while it is in fact healthy.
...
Plan-driven workflows (`service.IsPlanDriven`), after their static layers, self-draft or read the plan head: approved plans materialize into instance nodes and execution continues; otherwise the run suspends in a plan status (`planning`/`waiting_input`/`waiting_approval`) that stays non-terminal to callers. `ResumeAfterPlanApproval` resumes at the first materialized layer, or hands off to a live runLoop still drafting inline at the boundary (`ClaimPlanApprovalAtBoundary`), which then materializes and splices inline instead of suspending. Mechanics: REFERENCE.md § Plan Boundary & Materialization. The planner defaults workers to cheap tier; `dynwf_max_premium_workers` (default 2) caps premium (opus/fable) nodes — interactive approve rejects a premium-heavy plan, mode=auto downgrades it and writes a `_plan_premium_downgrade` warning finding (`service.EnforcePremiumWorkerCap`/`PlanService.ApproveAuto`).
