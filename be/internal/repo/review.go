...
// ReviewRepo handles CRUD for review_items
...
func NewReviewRepo(database db.Querier, clk clock.Clock) *ReviewRepo {
	return &ReviewRepo{db: database, clock: clk}
}

// Insert creates a new review item. Sets ID and timestamps; defaults status to pending.
func (r *ReviewRepo) Insert(item *model.ReviewItem) error {
...
func (r *ReviewRepo) Get(id string) (*model.ReviewItem, error) {
	row := r.db.QueryRow(`
...
func (r *ReviewRepo) List(projectID, status string, limit, offset int) ([]*model.ReviewItem, error) {
	var rows *sql.Rows
	var err error
...
func (r *ReviewRepo) UpdateDraft(id, projectID, draft string) error {
	now := r.clock.Now().UTC().Format(time.RFC3339Nano)
	result, err := r.db.Exec(`
		UPDATE review_items SET draft = ?, updated_at = ?
...
func (r *ReviewRepo) Approve(id, projectID string) error {
	now := r.clock.Now().UTC().Format(time.RFC3339Nano)
	result, err := r.db.Exec(`
		UPDATE review_items
		SET status = 'approved',
...
func (r *ReviewRepo) Reject(id, projectID, reason string) error {
	now := r.clock.Now().UTC().Format(time.RFC3339Nano)
	result, err := r.db.Exec(`
		UPDATE review_items
...
func scanReviewItem(row rowScanner) (*model.ReviewItem, error) {
...
func scanReviewItemRow(rows *sql.Rows) (*model.ReviewItem, error) {
	item := &model.ReviewItem{}
	var createdAt, updatedAt string
	var approvedAt *string
