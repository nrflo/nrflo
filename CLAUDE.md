...
29. **Low consumption mode**: Global setting (`GET/PATCH /api/v1/settings`) stored in the `config` table. When enabled, the spawner substitutes agents that have a `low_consumption_agent` configured in their agent definition. The substitute agent's prompt template, model, and timeout are used, but session records (agent_type, phase tracking) retain the original agent type. The setting is read once at workflow start — toggling mid-workflow has no effect until the next run. See [be/internal/spawner/CLAUDE.md](be/internal/spawner/CLAUDE.md) for substitution flow details.
