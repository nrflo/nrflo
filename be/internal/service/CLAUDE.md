...
| `dynamic_seed.go` (+ `_data.go`) | `EnsureGlobalDynamicWorkflow`: seeds the bundled global `dynamic` plan-driven workflow (fanout_template agents only, no static phases) |
