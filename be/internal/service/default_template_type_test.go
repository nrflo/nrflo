...
func TestDefaultTemplate_List_FilterByTypeInjectable(t *testing.T) {
	...
	if len(templates) != 13 {
		t.Fatalf("List(injectable) len = %d, want 13", len(templates))
	}
...
func TestDefaultTemplate_List_NoFilterReturnsAll(t *testing.T) {
	...
	if len(templates) != 19 {
		t.Fatalf("List() len = %d, want 19", len(templates))
	}
	...
	if injectableCount != 13 {
		t.Errorf("injectable count = %d, want 13", injectableCount)
	}
}
...
func TestDefaultTemplate_List_FilterAfterCreatingMixed(t *testing.T) {
	...
	if len(injectables) != 14 {
		t.Errorf("List(injectable) len = %d, want 14 (13 seeded + 1 created)", len(injectables))
	}
	...
	if len(all) != 21 {
		t.Errorf("List() len = %d, want 21 (19 seeded + 2 created)", len(all))
	}
}
