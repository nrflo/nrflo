...
func TestSetupInteractivePreStep_PlanMode_CreatesSession(t *testing.T) {
	...
	modelConfigs := map[string]spawner.ModelConfig{
		"opus-5-5": {Provider: "anthropic", CLIModel: "claude-opus-5-5"},
	}
	...
	if !strings.Contains(argsStr, "--model claude-opus-5-5") {
		t.Errorf("args missing mapped model --model claude-opus-5-5 (got nrflo ID instead): %v", registeredArgs)
	}
...
func TestSetupInteractivePreStep_PlanMode_DBMappedModelOverrides(t *testing.T) {
	...
	modelConfigs := map[string]spawner.ModelConfig{
		"opus-5-5": {Provider: "anthropic", CLIModel: "claude-opus-db-override"},
	}
	...
	if strings.Contains(argsStr, "--model opus-5-5") {
