...
// NrvappConfigVersionRepo handles versioned config snapshots
type NrvappConfigVersionRepo struct {
	db    *sql.DB
	clock clock.Clock
}

// NewNrvappConfigVersionRepo creates a new NrvappConfigVersionRepo.
// Accepts *sql.DB directly because Insert requires a transaction.
func NewNrvappConfigVersionRepo(database *sql.DB, clk clock.Clock) *NrvappConfigVersionRepo {
	return &NrvappConfigVersionRepo{db: database, clock: clk}
}

// Insert saves a new config version, auto-incrementing version per (project_id, file).
func (r *NrvappConfigVersionRepo) Insert(v *model.NrvappConfigVersion) error {
	...
	result, err := tx.Exec(`
		INSERT INTO nrvapp_config_versions (project_id, file, version, content, actor, created_at)
		VALUES (?, ?, ?, ?, ?, ?)`,
...
func (r *NrvappConfigVersionRepo) LatestVersion(projectID, file string) (int, error) {
	var version int
	err := r.db.QueryRow(`
...
func (r *NrvappConfigVersionRepo) Get(projectID, file string, version int) (*model.NrvappConfigVersion, error) {
...
func (r *NrvappConfigVersionRepo) History(projectID, file string) ([]*model.NrvappConfigVersion, error) {
	rows, err := r.db.Query(`
		SELECT id, project_id, file, version, content, actor, created_at
		FROM nrvapp_config_versions
		WHERE project_id = ? AND file = ?
	...
	var versions []*model.NrvappConfigVersion
	for rows.Next() {
		v := &model.NrvappConfigVersion{}
		var createdAt string
