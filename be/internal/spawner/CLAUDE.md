...
| `opencode` | `OpencodeAdapter` | `provider/model` (auto-mapped) | Generated by CLI | ✗ (batch only — see backlog) |
...
- `SupportsInteractive()` — true for Claude and Codex; **false for Opencode** (no observable event channel in opencode 1.14.48, see backlog).
