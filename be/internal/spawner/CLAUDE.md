...
Codex profile + `-c` hook injection: see `cli_adapter_codex_hooks.go`. The rollout JSONL tailer (`cli_adapter_codex_jsonl_tail.go`) records agent_messages and context_left out-of-band of hooks, bypassing the openai/codex#21639 PTY hooks regression.
