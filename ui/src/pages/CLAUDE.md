...
| `/settings` | `SettingsPage.tsx` | Tabbed settings page (General, Menu Panel, Projects, System Agents, Default Templates, CLI Models, API Models, Logs, Connections, Administration) — admin-only, gated via `RequireAdmin` at route level; CLI Models tab has Claude / Codex provider sub-tabs via `?sub=<provider>`; API Models tab has Anthropic / OpenAI provider sub-tabs via `?sub=anthropic\|openai`; Administration tab has Users / Audit Log / Service Tokens sub-tabs via `?sub=users\|audit\|tokens`; Connections tab via `?tab=connections` renders `ConnectionsSection` from `src/components/settings/`; Projects tab is lifecycle-only (list/create/delete/active + Settings action that switches active project and opens `/project-settings`); per-project editing lives on `/project-settings`; section components in `src/components/settings/` |
