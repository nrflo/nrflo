...
| `MessageCategory` | `'system_notice'` and `'task_notification'` are synthetic categories (no real model/human authorship); `'text'`/`'user_input'` now mean real model output / real human input only |
