-- A deterministic Verified State channel (be/internal/handoff) now supplies
-- file paths, line numbers, ticket IDs and command strings alongside every
-- fold digest. Retune the _refinery prompt to stop asking the model to
-- spell those out — a small model measurably hallucinated near-miss paths
-- (e.g. "LogMessage.ts" for "LogMessage.tsx") when asked to. The fold stays
-- narrative-only: intent, reasoning, decisions, blockers, open questions.
-- Keeps the ## Task anchor rules from migration 000196 intact.
