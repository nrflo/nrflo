...
| `agent_session_lookup.go` | `GetSessionProjectID`/`GetSessionKind`: single-column session lookups used by hook handlers to scope behavior (e.g. `socket.handleNotification` gates the Notification→nudge path to `kind='workflow_agent'`) without the full joined `GetSessionByID` load |
| `agent_definition.go` (+ `_read.go`) | Agent definition CRUD; layer/execution_mode/validation_commands/consultant/native_tools/sandbox/system_template_id/tier validation. Mechanics: [REFERENCE.md](REFERENCE.md#agent-definition-validation-agent_definitiongo) |
| `stepwise.go` | `StepRejectionCap` (project/global config, `DefaultStepRejectionCap=5`); `ResultReasonStepsIncomplete`/`FindingKeyStepsIncomplete` consts for the force-fail guard when a stepwise cursor is short of its last step |
| `stepwise_read.go` | `BuildStepCursors` read model: per-node `StepCursorProgress{current_index,total,done,steps[]}` for the v4 workflow response |
| `agent_definition_steps.go` | `PromptModeStepwise`/`PromptModeFull`; `validateStepDefinitions`/`validateRequiredFindings`/`validatePathOverlap`/`validateStepChecks` for `agent_definitions.steps`. Schema + mechanics: [REFERENCE.md](REFERENCE.md#stepwise-step-definitions); engine-side validation: [stepengine/CLAUDE.md](stepengine/CLAUDE.md) |
| `trace.go` (+ `_lanes`/`_markers`/`_sublanes`/`agent_tool_span`) | Read-time trace for `GET /workflow-instances/{iid}/trace`: lanes merged by chain root, layer bands, markers from agent_messages/findings_history (tool spans close via payload `ended_at`) + `lifecycle` markers and nudge/stop-block counters from session columns, children via `parent_instance_id`; `_sublanes.go` groups delegate-worker/consult-child sessions under their caller's lane from the durable `delegations`/`consults` rows, returned separately as `TraceResponse.SubLanes` |
...
| `worktree.go` (+ `_context.go`, `_delegate.go`) | Git worktree lifecycle: Setup, MergeAndCleanup, Cleanup; Setup seeds untracked/gitignored agent context into the worktree (CLAUDE.md/AGENTS.md copied, `.claude` dirs symlinked, any depth, absent-only). `_delegate.go`: `SetupFromHEAD`/`CommitAndCollect` branch/commit a delegate worktree off live HEAD, no fetch — see [spawner/CLAUDE.md](../spawner/CLAUDE.md#delegate) |
| `system_agent_definition.go` (+ `_read.go`, `system_agent_chain.go`) | System agent definition CRUD (global). `ResolveAgentChain(def)`: validated `[]AgentChainEntry` chain — `def.Model` override wins as a single entry, else nullable `tier` selects the `tier_models` chain (position 0 = primary), inheriting the nearest lower populated tier |
| `default_template.go` | Default template CRUD (global, readonly enforcement) |
| `model.go` + `model_update.go` + `model_reasoning.go` (+ `model_inuse.go`) + `model_sort.go` | Unified model CRUD: one provider row enables CLI/API through non-empty mode model IDs and carries per-mode contexts/effort lists, one default effort, nullable per-MTok `price_*`, and nullable `release_date`. `IsValidModelForMode` validates enabled mode support for definitions; readonly rows only accept `default_effort`/`fallback_models`, fallback models are anthropic-only and capped at 3. `ModelInUseCheck` blocks disable/delete while any agent/system def, or observer setting (global/project `observer_model` config, `workflows.observer_model`) references the id; clearing a mode's model id is blocked per execution mode (observers count as cli). `SortModelsForPicker` orders the console catalog release-date DESC (unknown last), tie-broken by `PlanModelTierClass` |
| `custom_provider.go` (+ `_update.go`) | `custom_providers` registry (name/base_url/api_key/api_wire/enabled) for BYO OpenAI-compatible servers; `resolveProvider` (model.go) checks builtins then this registry (Rule 6); delete/disable blocked while any model references the name |
...
| `plan.go` (+ `plan_*`/`workflow_instance_nodes`.go) | Plan lifecycle: `PlanService` draft/revise/approve→materialize/cancel/TTL-sweep; schema+semantic validation; template-library availability recheck + `PlanTemplateChoices` (the tiered bindable library `get_subworkflow` returns); hash-stamped idempotent `Materialize`; server-owned reserved key `_workflow_plan`. Full mechanics: [REFERENCE.md](REFERENCE.md#plan-lifecycle-plango--siblings) |
...
| `tiering.go` (+ `_report`/`_apply`/`_cost.go`) | `TierMap`/`ClassifyRole`: tier-map source of truth + admin re-tier report/apply (`GET`/`POST /api/v1/admin/tiering-*`); skips customized (model≠seed & ≠recommended, so re-apply is idempotent)/consultant/hotfix-implementor defs; apply/seed also idempotently grant `delegate,get_delegation` tools to the T0/T1 executor roles (setup-analyzer/test-writer/implementor) per `TierTarget.GrantsDelegation`; `ProjectService.seedTieredWorkflows` (`project_seed.go`) births new projects tiered |
...
Global defs are stored under the reserved `GlobalProjectID` (`"__global__"`, `workflow_reserved.go`); storage/runnable-scratch-project mechanics: [REFERENCE.md](REFERENCE.md#global-workflow-storage).
