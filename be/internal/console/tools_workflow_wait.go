...
func (workflowWaitHandler) Spec() provider.ToolSpec {
	return provider.ToolSpec{
		Name:        "workflow_wait",
		Description: "Long-poll a workflow instance for progress. Blocks until the instance's state digest (status, current phase, per-phase statuses, active agent sessions, plan status) differs from since_digest or timeout_seconds elapses, then returns {changed, terminal, digest, state?} — a trimmed tracking projection (status, current_phase, phases[status+result], active_agents[node_id,agent_type,model_id,restart_count], plan[status+questions]), NOT the full v4 map. A timeout returns {changed:false, terminal:false, digest} with no state key. Usage: call workflow_wait(instance_id) after workflow_run for the baseline digest (phase_order/phase_layers/layer_policies included only on this first call), then loop with since_digest=<previous digest> until terminal=true. Terminal responses add workflow_final_result when the run emitted a finding under that exact key (there is no aliasing — a run whose last node emits a different key omits it; read that key back via get_subworkflow's result_key), on failure failure_reason, plus next_workflow_on_success when the definition chains a follow-up workflow (a new project-scoped instance). Only completed/failed are terminal: a plan-driven run parking at planning/waiting_input/waiting_approval wakes this call with changed=true but terminal=false, so break on state.status too or you will block through the plan boundary — then call get_subworkflow for the draft and revision (this tool reports plan status/questions only, never the manifest). Call workflow_get for the full state snapshot (findings, agent_history, etc).",
