...
| `useAgentSessionLogs.ts` | TanStack Query hook for paginated finished agent sessions (`GET /api/v1/agent-session-logs`). Key factory: `agentSessionLogKeys`. `agentSessionLogKeys.all` is invalidated on `agent.completed` WS event. |
