...
| `trace.go` (+ `_lanes`/`_markers`) | Read-time trace for `GET /workflow-instances/{iid}/trace`: lanes merged by chain root, layer bands, markers from agent_messages/findings_history (`categories`+`marker_limit` params), children via `parent_instance_id` |
