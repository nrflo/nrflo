-- Add groups column to workflows (JSON array of tag strings)
ALTER TABLE workflows ADD COLUMN groups TEXT NOT NULL DEFAULT '[]';

-- Add tag column to agent_definitions (single tag string)
ALTER TABLE agent_definitions ADD COLUMN tag TEXT NOT NULL DEFAULT '';

-- Add skip_tags column to workflow_instances (JSON array of skip tag strings)
ALTER TABLE workflow_instances ADD COLUMN skip_tags TEXT NOT NULL DEFAULT '[]';

-- Recreate agent_sessions table with 'skipped' added to status CHECK constraint
