...
The model writes two findings via `findings_add`, reads them back via
...
Expected PASS:
  ...
  - findings.alpha == 'red' and findings.beta == 'blue'
"""
...
in this exact order:
  ...
  2. `findings_add` with {"key": "beta",  "value": "blue"}
  3. `findings_get` with {"keys": ["alpha", "beta"]}
...
def run(ctx: Ctx) -> Result:
    ...
    if findings.get("alpha") != "red" or findings.get("beta") != "blue":
        return ("A06 findings tool", "FAIL",
                f"findings = {findings!r}, want alpha=red, beta=blue")
