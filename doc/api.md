...
| `get_subworkflow` | Poll a sub-workflow; terminal statuses include the result finding/failure reason, plan-boundary statuses include `{plan, revision, questions, templates}`. Input: `{instance_id, result_key?, wait_sec?}` |
