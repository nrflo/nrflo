...
func (workflowWaitHandler) Spec() provider.ToolSpec {
	return provider.ToolSpec{
		Name:        "workflow_wait",
		Description: "Long-poll a workflow instance for progress. Blocks until the instance's state digest (status, current phase, per-phase statuses, active agent sessions, plan status) differs from since_digest or timeout_seconds elapses, then returns {changed, terminal, digest, state?} — a trimmed tracking projection (status, current_phase, phases[status+result], active_agents[node_id,agent_type,model_id,restart_count], plan[status+questions]), NOT the full v4 map. A timeout returns {changed:false, terminal:false, digest} with no state key. Usage: after workflow_run, call workflow_wait(instance_id) once for the baseline digest (state includes phase_order/phase_layers/layer_policies only on this first call), then loop workflow_wait(instance_id, since_digest=<previous digest>) reporting each transition until terminal=true. Terminal responses add workflow_final_result and, on failure, failure_reason, plus next_workflow_on_success when the definition chains a follow-up workflow (started as a new project-scoped instance). Call workflow_get for the full state snapshot (findings, agent_history, etc). For polling from scripts outside MCP, GET /api/v1/tickets/{id}/workflow or /api/v1/projects/{id}/workflow with an Authorization: Bearer service token instead of reading the database.",
