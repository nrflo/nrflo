...
_Generated by `python3 manual_testing/run_suite.py` at 2026-06-04T17:47:00Z (wall 882.2s, providers: engine, claude, codex, python, api, openai_api)._

Edit `manual_testing/suite.md` to change the scenario catalogue. This file is overwritten on every suite run.

> **⚠️ Stale — predates the v0.7.3 release (last full run 2026-06-04, nrflo 0.7.0).** The recorded `claude` (2.1.162) and `nrflo` (0.7.0) versions are out of date: the bundled Claude Code CLI now ships **2.1.178** and the current release is **0.7.3**. The scenario matrix itself is unchanged by v0.7.3 — no scenarios were added, and the rate-limit-reset, `--fallback-model`, and Stop-hook features are covered by existing `engine`/`claude`/`api` scenarios (all green on 2026-06-16, minus the OpenAI `codex`/`openai_api` folders, which need credentials). Re-run `python3 manual_testing/run_suite.py` — or `/pre-release` (needs OpenAI creds for the `codex`/`openai_api` columns) — to refresh.
