...
`nrflo_server console` is a thin native client in `internal/consoleui`: the server-owned catalog supplies available engines/models and live resumable chats. Trusted socket methods `console.chat`/`console.attach` return a scoped bearer locally, while remote clients use a service token; detaching leaves the engine live, explicit close tears it down, and reconnect detail restores bounded live deltas/thinking plus turn/approval state.
...
The engine name a chat row was started with is persisted in `agent_sessions.console_engine`, so `GET /api/v1/console/chats` does not depend on the in-memory session map. `pumpChatEvents` is the single writer of the `console_chat.approval_resolved`/`console_chat.thinking` session pushes for both engines — an engine's `RequestApproval`/`ReplyApproval`/resolved-notification paths only emit `spawner.EventApprovalResolved`, exactly once per registered id (whoever *drops* the pending entry owns the emit, so a reply racing a timeout cannot double-resolve) and never push WS/audit themselves. The REST reply route accepts `allow`/`allow_for_session`/`deny`; the pump normalizes the spawner's resolved decisions down to `allow`/`deny` on the WS push (codex maps `allow_for_session` natively, the claude engine keeps a server-side per-tool-name allowlist); token usage is pushed there too, but a claude chat's also fans out from `socket/handler_context.go`, so that one event legitimately arrives twice (idempotent).
