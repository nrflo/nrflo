...
These bullets describe the **PTY** path (claude/opencode/gemini); codex uses the app-server backend instead. `writeCodexProfileForSession` (`cli_adapter_codex_profile.go`) writes the per-session `CODEX_HOME` (auth + a `[projects."<path>"] trust_level="trusted"` entry — codex 0.133 reads workdir trust from `CODEX_HOME/config.toml`, and without it the session blocks on a directory-trust dialog); it is now called by the app-server backend's `Start`.
