...
| `MessageCategory` | `'system_notice'`, `'task_notification'` and `'system_turn'` (a server-authored chat wake-up — a user turn to the model that nobody typed) are synthetic categories (no real model/human authorship); `'text'`/`'user_input'` now mean real model output / real human input only |
