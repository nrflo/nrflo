...
describe('layout mobile (isMobile=true)', () => {
  it('MOBILE_NODE_WIDTH constant is 240', () => {
    expect(MOBILE_NODE_WIDTH).toBe(240)
  })

  it('AGENT_NODE_WIDTH constant is 320 (desktop unchanged)', () => {
    expect(AGENT_NODE_WIDTH).toBe(320)
  })

  it('uses MOBILE_NODE_WIDTH=240 for measured.width on mobile', async () => {
    const nodes = [makeNode('agent1', 0)]
    const { nodes: layouted } = await getLayoutedElements(nodes, [], null, true)
    expect(layouted[0].measured?.width).toBe(240)
  })

  it('uses desktop AGENT_NODE_WIDTH=320 for measured.width when isMobile=false', async () => {
    const nodes = [makeNode('agent1', 0)]
    const { nodes: layouted } = await getLayoutedElements(nodes, [], null, false)
    expect(layouted[0].measured?.width).toBe(320)
  })

  it('horizontal spacing between same-layer mobile nodes is 240+30=270', async () => {
    const nodes = [
      makeNode('agent1', 0),
      makeNode('agent2', 0),
      makeNode('next', 1),
    ...
    expect(spacing).toBe(270) // MOBILE_NODE_WIDTH(240) + nodeNode(30)
  })
  ...
  it('horizontal spacing between same-layer desktop nodes is 320+60=380', async () => {
    ...
    const layer0 = [layouted[0], layouted[1]].sort((a, b) => a.position.x - b.position.x)
    expect(layer0[1].position.x - layer0[0].position.x).toBe(380) // 320+60
  })
  ...
  it('mobile 20px edge padding: MOBILE_NODE_WIDTH(240) - visual card(220) = 20', () => {
    const VISUAL_CARD_WIDTH = 220
    expect(MOBILE_NODE_WIDTH - VISUAL_CARD_WIDTH).toBe(20)
  })
})
