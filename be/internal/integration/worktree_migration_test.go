...
func TestBuildV4State_IncludesWorktreeFields(t *testing.T) {
	...
	if err := wfiRepo.UpdateWorktree(wfiID, "/tmp/nrworkflow/worktrees/WK041-1", "WK041-1"); err != nil {
	...
	} else if wtp != "/tmp/nrworkflow/worktrees/WK041-1" {
		t.Errorf("worktree_path = %q, want /tmp/nrworkflow/worktrees/WK041-1", wtp)
	}
...
func TestWorktreeFieldsPersistAcrossReads(t *testing.T) {
	...
	wfiRepo := repo.NewWorkflowInstanceRepo(env.Pool, env.Clock)
	if err := wfiRepo.UpdateWorktree(wfiID, "/tmp/nrworkflow/worktrees/WK041-3", "WK041-3"); err != nil {
		t.Fatalf("UpdateWorktree: %v", err)
	}
	...
	for i := 0; i < 3; i++ {
		...
		if result["worktree_path"] != "/tmp/nrworkflow/worktrees/WK041-3" {
			t.Errorf("read %d: worktree_path = %v, want /tmp/nrworkflow/worktrees/WK041-3", i, result["worktree_path"])
		}
...
func TestWorktreeFieldsMarshalJSON(t *testing.T) {
	...
	if err := wfiRepo.UpdateWorktree(wfiID, "/tmp/nrworkflow/worktrees/WK041-4", "WK041-4"); err != nil {
		t.Fatalf("UpdateWorktree: %v", err)
	}
	...
	if after["worktree_path"] != "/tmp/nrworkflow/worktrees/WK041-4" {
		t.Errorf("worktree_path in JSON = %v, want /tmp/nrworkflow/worktrees/WK041-4", after["worktree_path"])
	}
