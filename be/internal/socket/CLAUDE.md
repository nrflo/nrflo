...
- `console.session` — mint a `kind='console'` session over the socket (filesystem access = auth), returning `{session_id, token, project_id, ticket_id}`; lets `nrflo_server console`/`agent mcp-external` reach a LOCAL server with no service token (remote keeps the HTTP token path). Resolves project explicit-hint → `cwd` → global
