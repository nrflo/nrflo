...
Every session-resolving path — `ListTools`/`CallTool` (`orchestrator_tools.go`) and the four control paths sharing `spawnerForSession` (`orchestrator_control.go`) — resolves through `resolveSessionSpawner`: the run's own `runState.spawners`, then `auxSpawners` for one-off children no run loop owns. The planner needs the second hop in both directions, since it is `cli_interactive` (tools arrive over the socket bridge, heartbeats via `record-event`) and a `revise_plan` re-run happens after `runLoop`'s defer already deleted the `runState`. Register a one-off child there via `OnSessionRegister`/`OnSessionUnregister`, never by synthesizing a `runState` — `IsRunning`/`StopWorkflow` act on that map. Missing the registration is silent: tools/list returns `[]` and every hook bump is a no-op, which surfaces as an agent stalling at `stall_start_timeout_sec` while it is in fact healthy.
