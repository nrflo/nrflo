...
// handoffTemplateDBPath holds a pre-migrated DB created once by TestMain.
// Every DB-backed test copies this file instead of migrating per test
// (be/CLAUDE.md: DB tests never migrate per-test).
var handoffTemplateDBPath string

func TestMain(m *testing.M) {
	dir, err := os.MkdirTemp("", "handoff-template-*")
	if err != nil {
		panic("create template dir: " + err.Error())
	}
	handoffTemplateDBPath = filepath.Join(dir, "template.db")
	d, err := db.OpenPath(handoffTemplateDBPath)
	if err != nil {
...
func newTestPool(t *testing.T) *db.Pool {
	...
	if err := copyHandoffTemplateDB(handoffTemplateDBPath, dest); err != nil {
...
func copyHandoffTemplateDB(src, dst string) error {
	in, err := os.Open(src)
	if err != nil {
		return err
	}
