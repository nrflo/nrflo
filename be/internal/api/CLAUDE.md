...
| `handlers_orchestrate.go` | Ticket-scoped run/stop/restart/retry-failed/take-control/resume-session/exit-interactive/run-epic. Run and retry-failed validate ticket is not blocked (409 Conflict) |
...
| `handlers_notification_channels.go` | Notification channel CRUD + /test + deliveries list (workflow-scoped, nested under /api/v1/workflows/{wid}/...; X-Project header required); secrets masked in responses. Also: GET /api/v1/notification-channels/variables (global, no project scope) |
...
| `handlers_insights.go` | Insights aggregations (summary, edit-rate, throughput); project-scoped; api-mode only |
...
# Project env vars (nested under projects; writes admin-only)
GET    /api/v1/projects/{id}/env-vars              # List all env vars for project (ordered by name ASC)
PUT    /api/v1/projects/{id}/env-vars/{name}       # Upsert; body: {value}; 400 on invalid name/reserved/oversized value; broadcasts project.env_vars_updated
DELETE /api/v1/projects/{id}/env-vars/{name}       # Delete; 404 if missing; broadcasts project.env_vars_updated
...
# Review / Config / Insights (api-mode only; all require X-Project header)

## Review items
GET    /api/v1/review                    # List review items; ?status=pending|approved|rejected, ?limit=, ?offset=
POST   /api/v1/review                    # Create review item; body: {tool_name, input, session_id?, output?, draft?}
GET    /api/v1/review/{id}               # Get one; includes diff field when draft is set ({added,removed,changed})
PATCH  /api/v1/review/{id}               # Update draft; body: {draft}; broadcasts review.updated
POST   /api/v1/review/{id}/approve       # Approve (copies draft→output if output empty); broadcasts review.updated
POST   /api/v1/review/{id}/reject        # Reject; body: {reason}; broadcasts review.updated

## Config editor (customer_config_dir must be set in project settings)
GET    /api/v1/config-files                        # List managed files (manifest + tool config_files + disk yaml/json)
GET    /api/v1/config-files/content/{file...}      # Get file content (DB version if edited, else disk fallback) + version number
PUT    /api/v1/config-files/content/{file...}      # Update file; raw body = new content; validates against sidecar schema; broadcasts config_file.updated
GET    /api/v1/config-files/history/{file...}      # Version history (newest first)
POST   /api/v1/config-files/rollback/{file...}     # Rollback; body: {version: int}; creates new version with old content; broadcasts config_file.updated

## Insights
GET    /api/v1/insights/summary          # Dispatch stats + review counts; ?range=7d|30d (default 7d)
...
# Notification channel variables (global, no project scope)
GET    /api/v1/notification-channels/variables    # Returns {variables:[...], defaults:{slack,telegram}}; protected, no admin, no X-Project header
