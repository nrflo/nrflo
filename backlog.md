...
- `codex resume [SESSION_ID]` — interactive resume by UUID, or `--last` for most recent
- `codex exec resume [SESSION_ID]` — non-interactive resume

The thread_id we'd need is already captured: `output_codex.go` extracts it from the `thread.started` event into `externalSessionID` and persists it. So the data is on hand; only the resume command wiring is missing.

Implementing resume gives us a faster, cheaper context save for codex (skip the haiku reconstruction call) when the resume mechanic works, with the existing agent-path remaining as the floor when it doesn't.

### Design
1. Flip `CodexAdapter.SupportsResume()` to `true`.
2. Implement `CodexAdapter.BuildResumeCommand(opts ResumeOptions) *exec.Cmd` — invokes `codex exec resume <opts.SessionID>` with stdin-piped save prompt, preserving the same `--dangerously-bypass-approvals-and-sandbox`, `-c check_for_update_on_startup=false`, model, and reasoning-effort flags used in `BuildCommand`/`BuildInteractiveCommand`. SessionID maps to codex's `thread_id` (we already store this as `agent_sessions.external_session_id`).
3. `context_save_resume.go` needs no changes — `shouldUseAgentSave` already routes to the resume path when `adapter.SupportsResume() == true`.
4. Verify the save prompt successfully elicits a `to_resume` finding from codex in the resumed session. Codex's hook regression (openai/codex#21639) doesn't apply here — `agent finished` / findings writes go through the nrflo CLI invoked from within the resumed session, not through codex's hook subprocesses.
5. Add coverage in `cli_adapter_codex_test.go` for `BuildResumeCommand` shape (args, env, stdin).

### Open questions
- Does `codex exec resume` reliably pre-load the prior turn context as the user-visible first turn, or does it require the prompt to re-state context? (Behavior may differ from `codex resume` interactive.) If batch resume is degraded, we may want to gate this behind `cli_interactive` only.
- Does the resumed session retain the per-session CODEX_HOME profile dir (`writeCodexProfileForSession`), or does it need to be re-staged? The profile is created at spawn time and cleaned up on session end; a resume relaunches the process, so cleanup ordering needs review.
- What's the right fallback when resume fails mid-save (e.g., codex exits with non-zero)? Probably the same as Claude: fall through to the agent path. Match the existing `contextSaveViaResume` failure behavior.

### Out of scope
- Resume for `cli_interactive` codex sessions specifically. Take-control already uses viewer-attach (no kill); the resume path is only relevant for the kill+relaunch context-save flow.
- Opencode resume. Same CLI-side limitation (no `--resume` equivalent that we've found); revisit if opencode adds it.
...
`ferryPTYOutput` (`be/internal/spawner/backend_interactive_helpers.go`) does three jobs for an interactive Claude session:

1. **Drain the PTY master** — necessary; if the bytes aren't consumed the kernel buffer (a few KB) fills and the Claude CLI blocks on write, deadlocking the session. Cannot be removed.
2. **Close `firstByteCh` once** — necessary; `deliverPrompt` waits on this before writing the prompt.
3. **Bump `proc.lastMessageTime` on every chunk** — questionable.

The third one is what makes stall detection unreachable under PTY. Measured 2026-05-12 with a temporary per-second byte counter (`s16_stall_detection` under cli-interactive, agent runs `sleep 30`): the Claude TUI's status-line / "Running (Ns ⋅ ↑ ↓)" timer redraw produces **11–25 PTY chunks, 700–3000 bytes per second, sustained for the entire 30s sleep window**. No quiet second. Every chunk bumps `lastMessageTime`, so the 15s running-stall threshold can never accumulate. `s16` FAILs cleanly under cli-interactive without the skip guard — the agent completes normally at T+40s with `result=pass`, never triggering `stall_running_timeout`.

The code comment at line 127 calls this out as intentional: *"lastMessageTime is bumped on each chunk so stall detection doesn't fire while the agent is redrawing."* That rationale is backwards — redraws aren't agent progress, they're terminal noise. For Claude we already have a canonical "agent is doing something" signal: hooks (`PreToolUse`, tool_result, `Stop`, `session.idle` for opencode) bump `lastMessageTime` via `BumpLastMessage` from `socket/handler_record_event.go`. Treating TUI noise as activity defeats the timer that's supposed to catch a wedged session.

### Design

Add a per-adapter opt-out:
...
type CLIAdapter interface {
    ...
    // BumpsOnPTYBytes reports whether ferryPTYOutput should treat raw PTY
    // bytes as an "agent making progress" signal for stall detection.
    // Adapters that have a reliable structured-activity channel (Claude
    // via hooks, Opencode via SSE) return false. Codex returns true while
    // the upstream hooks regression (openai/codex#21639) forces us to use
    // captureTUIBytes; flip back to false when that's resolved.
    BumpsOnPTYBytes() bool
}
```

Per adapter:
- `ClaudeAdapter.BumpsOnPTYBytes()` → `false`
- `OpencodeAdapter.BumpsOnPTYBytes()` → `false` (SSE bus drives `BumpLastMessage`)
- `CodexAdapter.BumpsOnPTYBytes()` → `true` (while `CapturesTUIBytes()` is on; lift to `false` when codex#21639 is fixed)

`ferryPTYOutput` reads it once per chunk; when false, drain bytes + close firstByteCh + (codex only) DSR replies / captureTUI, but **do not touch** `proc.lastMessageTime` or `proc.hasReceivedMessage`. `hasReceivedMessage` then flips on the first **hook** instead of the first PTY byte — which is actually a better proxy for "agent has started working" since cursor-position bytes from `claude --resume` startup aren't agent activity.

### Consequences

**Wins:**
- `s16_stall_detection` becomes reachable under `cli-interactive`; skip can be lifted.
- A genuinely wedged Claude PTY session (model produces no tool call for `stall_running_timeout` seconds, default 8 min) now triggers stall + auto-relaunch instead of running until the workflow-level timeout.
- The `start_stall` distinction sharpens: it fires when no hook arrives within `stall_start_timeout` (default 2 min), catching cases where the CLI launched but the model never produced anything (auth wedge, model unreachable, etc.).

**Risks / open questions:**
- The model "thinking" between two tool calls produces no hooks. For haiku/sonnet thinking is usually sub-second; for opus 4.7 on a hard problem it can be 30–90s. The default `stall_running_timeout=8 min` is well above that, but `s16`'s 15s override would be too aggressive for real workflows — that's fine, s16 is a stall test, not a model-thinking test.
- Codex stays opt-in for the bump until upstream ships a fix (per `CapturesTUIBytes`).
- Idle/nudge loop (`checkIdleNudge`) reads the same `lastMessageTime` for its own window. Verify the change doesn't make idle/nudge fire prematurely while the model is thinking — likely fine since `idleStartTimeout` defaults to 2 min and `idleAfterMessageTimeout` to 3 min, both safely above realistic thinking spans.

### Surface area

- `be/internal/spawner/cli_adapter.go` — add `BumpsOnPTYBytes() bool` to `CLIAdapter`.
- `be/internal/spawner/cli_adapter_claude.go` — return `false`.
- `be/internal/spawner/cli_adapter_opencode.go` — return `false`.
- `be/internal/spawner/cli_adapter_codex.go` — return `true` (tie to `CapturesTUIBytes`).
- `be/internal/spawner/backend_interactive_helpers.go` — read flag, skip the bump and `hasReceivedMessage` flip when false. Still always drain bytes.
- `be/internal/spawner/CLAUDE.md` — update "Output Ferry" section.
- `manual_testing/scenarios/s16_stall_detection.py` — drop the `cli-interactive` skip; expected behavior becomes PASS under all modes for Claude.

### Verification

- Existing unit tests in `output_test.go` / interactive backend tests stay green.
- Add a focused test: spawn an interactive-mode proc with `BumpsOnPTYBytes()=false`, feed PTY bytes through a mock `sess.Read`, advance `clock.TestClock` past `stallRunningTimeout`, assert `checkStall` returns true.
- Re-run `manual_testing/test_claude.py --mode=cli-interactive` — s16 should pass; rest should stay green (no real workflow gets near the stall threshold).

### Out of scope

- Opencode SSE bus driving the bump correctly — already works (`session.idle` already calls `BumpLastMessage`); just stop bumping from PTY bytes too.
- Codex bump removal — gated on the upstream hooks regression closing.
