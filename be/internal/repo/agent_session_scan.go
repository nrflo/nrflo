...
const sessionCols = `id, project_id, ticket_id, workflow_instance_id, phase, node_id, agent_type,
	...
	rate_limit_retry_count, rate_limit_until_ts, last_retry_class, kind, observer_scope, observer_workflow_id, console_engine, console_profile`
...
func scanSession(scanner interface{ Scan(...interface{}) error }) (*model.AgentSession, error) {
	...
	err := scanner.Scan(
		...
		&s.RateLimitRetryCount, &s.RateLimitUntilTs, &s.LastRetryClass, &s.Kind, &s.ObserverScope, &s.ObserverWorkflowID, &s.ConsoleEngine, &s.ConsoleProfile,
	)
	if err != nil {
		return nil, err
	}
