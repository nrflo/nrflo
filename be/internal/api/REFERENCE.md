...
**Startup**: resolve the session project — **cwd auto-detect** (the bridge's working directory matched against project `root_path`s, longest-prefix; `cli/agent_mcp_external_cwd.go`) → `NRFLO_PROJECT` default → the hidden global project (never errors) — then exchange the service token for a console session via `POST /api/v1/console/sessions` (service token, `X-Project: <resolved project>`, body `{ticket_id: <git branch>}`) → `{session_id, token, ticket_id}` — the accepted current ticket, validated server-side (`''` when the branch is not a known ticket). A **global** service token can open a session for any project; a **project** token only its own (403 otherwise, surfaced with the resolved project id for diagnosis). The resolved project is fixed for the life of the connection — console tool schemas carry no `project` arg.

**Adopted-session path**: when `NRFLO_CONSOLE_TOKEN`/`NRFLO_CONSOLE_SESSION_ID` are both set by a server-owned console-chat engine, startup skips `POST /api/v1/console/sessions` — `adoptConsoleSession` installs the bearer and pins `NRFLO_PROJECT`. `ownsSession` stays false because `ChatService` owns close; if the bearer expires, a configured service token can open a replacement tool session over HTTP, while a local tokenless engine can re-mint over the trusted Unix socket.
...
`POST /api/v1/console/sessions` (`projectAdmin`; project from X-Project) creates a `kind='console'` `agent_sessions` row (`status=user_interactive`, NULL `workflow_instance_id`) and returns `{session_id, token, ticket_id}` once. An optional body `{ticket_id}` (the caller's git branch) is the session's current-ticket hint: validated against the project via `TicketService.Get`, stored on the row when it resolves, dropped to `''` otherwise (never a 4xx). The response echoes the accepted id (read back by the `ticket_current` console tool; `''` when none). `POST /api/v1/console/sessions/{sid}/close` (`protected`) authorizes in-handler (admin, OR service principal Global/project-match, OR the session's own bearer) then flips `status=interactive_completed`, killing the token via `GetByToken`'s status filter. Idle rows sweep every 20 min via `ConsoleService.SweepIdle` (`console_idle_ttl_hours`, default 12).

#### Console tools

`GET /api/v1/console/tools` and `POST /api/v1/console/tools/{name}/call` (both `protected`) expose the `internal/console/` tool profile to a `console` or `console_chat` bearer. Both handlers call `requireConsoleSession(r)` first; closed tokens and unrelated bearer kinds resolve to 401.
