...
| `provider_settings.go` | `ProviderSettingsService`: GetModes/SetModes/GetAll; per-provider CLI execution-mode allowlist stored as comma-joined values under `provider_<name>_modes` keys; defaults to `[cli]` for opencode and `[cli, cli_interactive]` for claude/codex; opencode rejects `cli_interactive` in SetModes |
