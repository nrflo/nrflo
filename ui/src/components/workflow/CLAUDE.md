...
| `AgentsTable.tsx` | Flat table view (simplified mode). Columns: Agent (name + emerald tag badge), Level, Model, Run mode (`cli`\|`cli interactive`\|`api`\|`script`\|`—`), Status (StatusCell + optional `(interactive control)` indicator + amber nudge badge ⏰N/5 when running + Retry icon button when `result=fail` AND `workflowStatus=failed`), Attempts (plain or Tooltip with restart reasons when restart_count>0), Context left (AlertTriangle + amber color when running and `context_left <= restart_threshold`, otherwise `contextLeftColor`), Duration. Props: `onRetryFailed`, `retryingSessionId`, `workflowStatus` forwarded from PhaseTimeline (same props as PhaseGraph). Retry uses a single table-level ConfirmDialog (confirmSessionId state). |
...
| `AgentDefForm.tsx` | Agent definition create/edit form (includes layer input, tag dropdown when groups available, "Apply Template" button opens TemplatePickerDialog). Execution Mode dropdown always visible; options: `cli` (default), `cli_interactive` (PTY), `api` (only when `apiModeEnabled`), `script`. When `executionMode==='script'`: hides Model, Low consumption model, AgentDefAPIModeFields, and Prompt Template; renders `PythonScriptPickerField` (required). `cli_interactive` behaves identically to `cli` for field visibility (model/prompt/restart/low-consumption shown). On mode switch away from script, clears `pythonScriptId`. Script-mode submit sends `{python_script_id, layer, timeout, max_fail_restarts, tag, execution_mode:'script'}` omitting prompt/model/tools/api_max_iterations/low_consumption_model. |
