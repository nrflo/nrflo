...
func (s *Server) handleNrvappInsightsSummary(w http.ResponseWriter, r *http.Request) {
	...
	svc := service.NewNrvappInsightsService(s.pool, s.clock)
...
func (s *Server) handleNrvappInsightsEditRate(w http.ResponseWriter, r *http.Request) {
	projectID := getProjectID(r)
	if projectID == "" {
		writeError(w, http.StatusBadRequest, "X-Project header required")
		return
...
func (s *Server) handleNrvappInsightsThroughput(w http.ResponseWriter, r *http.Request) {
	projectID := getProjectID(r)
	if projectID == "" {
		writeError(w, http.StatusBadRequest, "X-Project header required")
		return
