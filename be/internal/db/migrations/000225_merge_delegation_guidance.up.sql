-- merge_delegation lands executor branches server-side: point the readonly
-- delegation-guidance injectable's final bullet at the tool instead of a
-- manual `git merge`, which callers were performing by escaping their
-- isolated worktree into the live tree (invisible to the delegation record).
