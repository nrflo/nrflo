...
// ensureSpawnerTempDir creates /tmp/nrflo so the spawner can write temp files.
func ensureSpawnerTempDir(t *testing.T) {
	t.Helper()
	if err := os.MkdirAll("/tmp/nrflo", 0o755); err != nil {
		t.Fatalf("failed to create /tmp/nrflo: %v", err)
	}
}
...
func TestAttemptConflictResolution_NoResolverConfigured(t *testing.T) {
	...
	err := env.orch.attemptConflictResolution(context.Background(), "wfi-no-resolver", req, wt, env.pool, "merge error text", spawner.Config{})
...
func TestAttemptConflictResolution_ResolvingEventBroadcast(t *testing.T) {
	...
	go func() {
		done <- env.orch.attemptConflictResolution(ctx, wfiID, req, wt, env.pool, mergeErrMsg, spawner.Config{})
	}()
...
func TestAttemptConflictResolution_SpawnFails_FailedEvent(t *testing.T) {
	...
	go func() {
		done <- env.orch.attemptConflictResolution(ctx, wfiID, req, wt, env.pool, "auto-merge failed", spawner.Config{})
	}()
...
func TestAttemptConflictResolution_ExtraVarsInResolvingEvent(t *testing.T) {
	...
	go func() {
		env.orch.attemptConflictResolution(ctx, wfiID, req, wt, env.pool, mergeErr, spawner.Config{})
	}()
