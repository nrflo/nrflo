...
`claudeEngine` drives a human console session over the same PTY + Claude-hooks transport the autonomous `cli_interactive` backend uses — no headless `-p`/`--output-format stream-json`, so a console session stays on the human's Claude subscription rather than metered API pricing. `Start` writes a per-session `--mcp-config` (`WriteConsoleClaudeMCPConfig`, shared with `console/driver_claude.go`) and `--settings` (`BuildConsoleSettingsJSON`), then `RegisterLaunch`+`Create`s a PTY session keyed by `spec.SessionID` (also a valid `claude --session-id`) and registers itself with the injected `*ConsoleHub`. Argv carries none of the managed-session flags: no `--dangerously-skip-permissions`, `--disallowedTools`, `--strict-mcp-config`, or safety-hook merge — the same boundary `console/driver_claude.go` documents for the local console driver. PTY output is read and dropped (`ferry`): claude's heartbeat and turn boundaries come from hooks, not bytes.
