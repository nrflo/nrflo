...
|----------|----|-----------|-------------|-------------|
| anthropic | `fable-5-1` | `claude-fable-5-1` | 1M | low, medium, high, xhigh, max |
...
| anthropic | `opus-5-5` | `claude-opus-5-5` | 1M | low, medium, high, xhigh, max |
| anthropic | `opus-5-5-1m` | `claude-opus-5-5[1m]` | 1M | low, medium, high, xhigh, max |
...
| openai | `gpt-6-astra` | `gpt-6-astra` | 1.05M | low, medium, high, xhigh, max |
| openai | `gpt-6-sol` | `gpt-6-sol` | 1.05M | low, medium, high, xhigh, max |
| openai | `gpt-6-luna` | `gpt-6-luna` | 1.05M | low, medium, high, xhigh, max |
...
**`dynamic_workflow` / `revise_plan` / `approve_plan`** drive an on-demand, plan-driven sub-workflow — the bundled global `dynamic` workflow (fanout_template agents only; a planner drafts its manifest). `dynamic_workflow` starts it as a child sharing `run_subworkflow`'s guards/caps; `mode: "approve"` (default) parks the child at `waiting_approval` for the caller to drive via `revise_plan`/`approve_plan`; `mode: "auto"` auto-approves and materializes without suspending — refused unless the `dynamic_workflow_auto_enabled` config key (project-override, else global; default off) is true. `get_subworkflow` on a child in one of the three plan-boundary statuses (`planning`/`waiting_input`/`waiting_approval`) returns `{plan, revision, questions, templates, premium_cap}` instead of a terminal result — `templates` is the bindable library (`id`/`description`/`model`/`tier`) a hand-written manifest must draw its node `template` names from, since a node carries no model/effort field of its own and premium-tier nodes are capped by `premium_cap` (the project's effective `dynwf_max_premium_workers`); `revise_plan`/`approve_plan` are revision-pinned (a stale `revision` errors, naming the current one). A hand-written manifest must satisfy `ValidatePlanManifest`: dense 0-indexed layers, plan-unique node ids, exactly one node in the final (result-carrying) layer, and — since a node sees only its own instructions — [`#{NODE_FINDINGS:<node-id>}`](common-20-findings.md#node-findings-node_findingsnode_id) references targeting a strictly earlier layer as the only inter-layer data path. Any `callable_as_subworkflow` workflow with no static layers self-drafts at the plan boundary the same way, independent of these tools — `mode`/auto-approve is persisted on the run as `plan_auto_approve`.
