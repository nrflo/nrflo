...
const scheduleRunCols = `id, scheduled_task_id, project_id, triggered_at, status, workflows, error`

func scanScheduleRun(row interface{ Scan(...interface{}) error }) (*model.ScheduleRun, error) {
	r := &model.ScheduleRun{}
	var triggeredAt, workflowsJSON string
	if err := row.Scan(&r.ID, &r.ScheduledTaskID, &r.ProjectID, &triggeredAt, &r.Status, &workflowsJSON, &r.Error); err != nil {
		return nil, err
	}
