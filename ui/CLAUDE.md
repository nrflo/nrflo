...
```
Layout
├── Header (project selector, search, navigation: Dashboard/Tickets/Workflows/Git/Documentation, daily stats, theme toggle, settings link)
...
- Settings page with tabbed layout (General, Projects, System Agents, Default Templates, CLI Models, Logs, Notifications, Administration): project management (create/update/delete, Toggle for git worktrees), system agent definitions CRUD (global agents like conflict-resolver), default template CRUD (readonly built-in templates + user-created templates with MarkdownEditor), CLI model CRUD (readonly built-in models + user-created models with cli_type badges and warnings), notification channels CRUD (per-project Slack/Telegram channels with kind badge, enabled toggle, event-type checkboxes, deliveries panel, Send Test button), Administration tab with Users / Audit Log sub-tabs (controlled by `?tab=administration&sub=users|audit`) containing user management CRUD and paginated audit log
...
- Python Scripts page: `PythonScriptsPage` at `/python-scripts` — CRUD for project-scoped Python scripts. Write controls admin-only (enforced server-side); `ReadOnlyHint` shown for non-admins. `PythonScriptForm` dialog (Name/Description/CodeEditor[python]) with Check Syntax button (calls `validatePythonScript`). Save flow: validate on submit → if ok call mutation; if syntax error open ConfirmDialog with error message and "Save anyway" option. Uses `usePythonScripts`, `useCreatePythonScript`, `useUpdatePythonScript`, `useDeletePythonScript`. Types in `src/types/pythonScript.ts`, API in `src/api/pythonScripts.ts`.
...
- Logs page: `LogsPage` at `/logs` shows paginated finished agent sessions (Finished/SID/Agent/Model/Mode/Workflow/Duration/Status/Result columns). CalendarClock icon in Workflow cell when `scheduled=true`. CheckCircle2 result icon with Tooltip when `workflow_final_result` present. Auto-refreshes on `agent.completed` WS event via `agentSessionLogKeys.all` invalidation. Types in `src/types/agentSessionLogs.ts`, API in `src/api/agentSessionLogs.ts`, hook in `src/hooks/useAgentSessionLogs.ts`.
