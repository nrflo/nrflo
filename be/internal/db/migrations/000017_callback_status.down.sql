-- Revert callback status: convert back to completed/pass, rebuild without callback in CHECKs.

PRAGMA foreign_keys = OFF;

-- Convert any callback rows back to valid values before rebuilding
UPDATE agent_sessions SET status = 'completed' WHERE status = 'callback';
UPDATE agent_sessions SET result = 'pass' WHERE result = 'callback';

-- Rebuild agent_sessions without callback in CHECK constraints
CREATE TABLE agent_sessions_new (
    ...
    status                TEXT NOT NULL DEFAULT 'running'
        CHECK (status IN ('running', 'completed', 'failed', 'timeout', 'continued', 'project_completed')),
    result                TEXT
