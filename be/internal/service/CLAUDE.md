...
`EnsureGlobalDeepResearch(pool, clk, rootPath)` (`deep_research_seed.go` + `_data.go`) idempotently ensures the `__global__` project exists **and** has a `root_path` (backfilled even on installs predating it being runnable), then create-if-absent seeds the bundled `deep-research` workflow (`is_global=1`, 6 `cli_interactive` agents — 5 claude + 1 codex GPT-5.5 `verify_b` — finding schemas, L2 `quorum:2`) via direct SQL at startup (`cli/serve.go` passes `<data>/global`), bypassing the service layer's model validation since it is shipped data. The agents run cli_interactive so the claude/codex CLIs self-authenticate (no server-side API credential needed); migration `000148` flips installs seeded under the older api-mode roster, `000149` tunes it from live-run analysis (verifiers get `web_fetch`+artifact access to reuse the researcher's cached pages, `angles` floor relaxed to 1, emit-or-fail completion contract in every prompt). Mutating `__global__` defs is admin-only — HTTP `api.denyNonAdminGlobalWrite`, socket `socket.denyGlobalWorkflowMutation`.
