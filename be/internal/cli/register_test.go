...
func TestRegisterServerCommands(t *testing.T) {
	...
	if rootCmd.Use != "nrworkflow_server" {
		t.Errorf("RegisterServerCommands: rootCmd.Use = %q, want 'nrworkflow_server'", rootCmd.Use)
	}
	if rootCmd.Short != "nrworkflow server" {
		t.Errorf("RegisterServerCommands: rootCmd.Short = %q, want 'nrworkflow server'", rootCmd.Short)
	}
...
func TestRegisterCLICommands(t *testing.T) {
	...
	// Check root command metadata
	if rootCmd.Use != "nrworkflow" {
		t.Errorf("RegisterCLICommands: rootCmd.Use = %q, want 'nrworkflow'", rootCmd.Use)
	}
	if rootCmd.Short != "CLI tool for nrworkflow server" {
		t.Errorf("RegisterCLICommands: rootCmd.Short = %q, want 'CLI tool for nrworkflow server'", rootCmd.Short)
	}
...
func TestNoCommandLeakage(t *testing.T) {
	...
	testRootCmd1 := &cobra.Command{Use: "nrworkflow"}
	...
	testRootCmd2 := &cobra.Command{Use: "nrworkflow"}
...
func TestAgentSubcommands(t *testing.T) {
	...
	expectedSubcommands := []string{"complete", "fail", "continue", "callback", "context-update"}
	...
	// Verify exact count: 5 subcommands
	if len(actualSubcommands) != 5 {
		t.Errorf("agentCmd has %d subcommands, want 5. Subcommands: %v", len(actualSubcommands), actualSubcommands)
	}
}
...
func TestRootCommandMetadata(t *testing.T) {
	...
	RegisterServerCommands()
	if rootCmd.Use != "nrworkflow_server" {
		t.Errorf("After RegisterServerCommands: Use = %q, want 'nrworkflow_server'", rootCmd.Use)
	}
	if rootCmd.Short != "nrworkflow server" {
		t.Errorf("After RegisterServerCommands: Short = %q, want 'nrworkflow server'", rootCmd.Short)
	}
	...
	expectedUse := "nrworkflow"
	expectedShort := "CLI tool for nrworkflow server"
