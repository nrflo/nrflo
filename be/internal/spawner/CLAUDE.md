...
| `opencode` | `OpencodeAdapter` | `provider/model` (auto-mapped) | Generated by CLI | ✗ (batch only; context tracking: cli (sqlite tail) — see backlog) |
...
- `SupportsInteractive()` — true for Claude and Codex; **false for Opencode** (cli_interactive not yet enabled; see backlog for follow-up ticket).
