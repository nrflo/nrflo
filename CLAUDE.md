...
37. **Interactive CLI backend**: When the per-project `interactive_cli_mode` setting is enabled, the spawner uses `cliInteractiveBackend` instead of `cliBackend` for CLI agents whose adapter returns `SupportsInteractive() == true` (all three: Claude, Opencode, Codex). The agent process is spawned inside a PTY (no batch flags); the rendered prompt is delivered via PTY stdin Write after a ~250ms readiness delay. Suffix delivery asymmetry: Claude uses `--append-system-prompt-file`, Opencode/Codex prepend the suffix to the prompt body in memory. Hook telemetry delivery split: Claude receives hooks via `--settings` JSON; Codex receives hooks via a per-session CODEX_HOME profile dir (config.toml + hooks.json written by `BuildCodexHookProfile`, injected as `CODEX_HOME=<dir>` env var, cleaned up on session exit); Opencode has neither. Take-control in this mode is a **viewer-attach** — the agent is not killed; `agent.viewer_attached` is broadcast and the viewer connects via the standard `/api/v1/pty/{session_id}` endpoint (which accepts `status=running` when a PTY session already exists). Viewer disconnect does not trigger `exit-interactive`. System agents (conflict-resolver, context-saver-CLI) inherit the same backend selector. `spawner.Config.PTYManager` carries the PTY manager from the orchestrator (`Orchestrator.PTYManager`, wired in `server.go`). See [be/internal/spawner/CLAUDE.md](be/internal/spawner/CLAUDE.md) for full backend selection rules, PTY lifecycle, and output ferry behavior.
