...
func (s *Server) handleListNrvappReviews(w http.ResponseWriter, r *http.Request) {
	...
	rr := repo.NewNrvappReviewRepo(s.pool, s.clock)
...
func (s *Server) handleCreateNrvappReview(w http.ResponseWriter, r *http.Request) {
	projectID := getProjectID(r)
	if projectID == "" {
		writeError(w, http.StatusBadRequest, "X-Project header required")
		return
...
type reviewWithDiff struct {
	*model.NrvappReviewItem
	Diff interface{} `json:"diff,omitempty"`
}

func (s *Server) handleGetNrvappReview(w http.ResponseWriter, r *http.Request) {
	projectID := getProjectID(r)
	if projectID == "" {
		writeError(w, http.StatusBadRequest, "X-Project header required")
		return
	...
	resp := reviewWithDiff{NrvappReviewItem: item}
	if item.Draft != nil {
...
func (s *Server) handlePatchNrvappReview(w http.ResponseWriter, r *http.Request) {
	projectID := getProjectID(r)
	if projectID == "" {
		writeError(w, http.StatusBadRequest, "X-Project header required")
		return
	...
	s.wsHub.Broadcast(&ws.Event{
		Type:      ws.EventNrvappReviewUpdated,
		ProjectID: projectID,
...
func (s *Server) handleApproveNrvappReview(w http.ResponseWriter, r *http.Request) {
	projectID := getProjectID(r)
	if projectID == "" {
		writeError(w, http.StatusBadRequest, "X-Project header required")
		return
...
func (s *Server) handleRejectNrvappReview(w http.ResponseWriter, r *http.Request) {
	projectID := getProjectID(r)
	if projectID == "" {
		writeError(w, http.StatusBadRequest, "X-Project header required")
		return
