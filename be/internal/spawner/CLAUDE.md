...
| `opencode` | `OpencodeAdapter` | `provider/model` (auto-mapped) | Generated by CLI | ✓ (TUI on free port; sqlite tail; context tracking: cli + cli_interactive) |
...
- `SupportsInteractive()` — true for Claude, Codex, and Opencode. Opencode interactive observability flows through the SQLite tailer (PostStart fires for both cli and cli_interactive).
