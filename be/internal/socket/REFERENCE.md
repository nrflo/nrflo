...
Deep mechanics for this package. The auto-loaded map lives in [CLAUDE.md](CLAUDE.md); read this file when changing method payloads, observer authorization, terminal-signal dispatch, or handler layout.

Contents: [Supported Methods](#supported-methods) · [Observer Authorization](#observer-authorization) · [Terminal Signal Dispatch](#terminal-signal-dispatch) · [Files](#files)
...
| `agent.record_event` | Record Claude hook event; PreToolUse (invoke) + PostToolUse (`[tool] → output` result) insert message rows + WS broadcast. PreToolUse stores `tool_use_id` in the row payload; PostToolUse/PostToolUseFailure close the tool span by stamping `ended_at` into that payload (`AgentService.MarkToolEnded`) for trace duration bars. PostToolUse for `spawner.IsHiddenResultTool` tools (Read/Bash/Edit) is dropped at the source (`status="ignored"`, no row; the span still closes) — the invoke row already names the file/command. Stop enforces completion (`handleStopHook`): for an autonomous `running` session with no completion-tool result, returns a `decision:block` + finish-reminder (up to `stopBlockCap` blocks) so the CLI keeps the agent going, then fails it (`unresponsive_after_stop_blocks`); sessions with a result or non-running status pass through. PreToolUse/PreCompact also tail the Claude transcript (`event["transcript_path"]`) into `category="thinking"` rows — see `transcript_thinking.go`. UserPromptSubmit routes through `ConsoleHooks.ConsoleUserPrompt(session, prompt)` — handled=true (the live engine's own SendUserTurn echo, matched once against its pending text) skips the insert since the engine already persisted the row; human-typed prompts from an attached console terminal return false and are recorded here. `handler_record_event_log.go` logs exactly one line per received event before dispatch: INFO for lifecycle events (SessionStart/Stop/SubagentStart/SubagentStop/StopFailure/PreCompact/Notification/PostToolUseFailure), DEBUG for high-volume ones (PreToolUse/PostToolUse/UserPromptSubmit/UserPromptExpansion/SessionEnd), WARN for unrecognized hook names and for rejected/failed calls (bad params, missing session_id, decode errors, or a dispatch error response); detail values are truncated to 120 chars and never include prompt text or tool payload bodies |
...
| `handler_record_event.go` | `dispatchRecordEvent`: PreToolUse (invoke) + PostToolUse (result) → DB insert + WS broadcast; Stop → no-op boundary ack |
| `handler_record_event_log.go` | `agent.record_event` ingress: decode params/event, WARN on rejection, one INFO/DEBUG log line per hook event, then `dispatchRecordEvent` |
