...
def run(ctx: Ctx) -> Result:
    # The agent-saver branch is reached automatically only on adapters
    # whose SupportsResume() == false. Today that's opencode. Other
    # providers route through contextSaveViaResume and are covered by s26.
    if ctx.provider != "opencode":
        return ("S27 context save (agent saver)", "SKIP",
                f"{ctx.provider} adapter supports resume; agent-saver "
                "branch covered only via opencode in stock config")
