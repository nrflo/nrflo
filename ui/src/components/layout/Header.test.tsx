...
describe('Header - Brand label', () => {
  ...
  it('falls back to N icon and nrflo text when no project is selected', () => {
    mockCurrentProject = ''

    renderHeader()

    const brandLink = screen.getByRole('link', { name: /nrflo/i })
    expect(brandLink).toBeInTheDocument()
    expect(brandLink).toHaveTextContent('N')
    expect(brandLink).toHaveTextContent('nrflo')
  })
...
describe('Header - Icon-only nav links', () => {
  ...
  it('all 6 nav links render with correct titles, hrefs, and responsive text labels', () => {
    ...
    const expectedLinks = [
      ...
      { title: 'Sessions', href: '/logs', label: 'Sessions' },
    ]
