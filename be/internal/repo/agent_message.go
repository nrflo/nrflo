...
// CountBySessionActionable returns the message count excluding init and thinking messages
// that are auto-generated by CLI startup and don't represent real agent work.
func (r *AgentMessageRepo) CountBySessionActionable(sessionID string) (int, error) {
	var count int
	err := r.db.QueryRow(
		`SELECT COUNT(*) FROM agent_messages WHERE session_id = ? AND content NOT LIKE '[init]%' AND content NOT LIKE '[thinking]%'`,
		sessionID,
	).Scan(&count)
	return count, err
}
...
// CountBySessionActionable returns the message count excluding init and thinking messages
// that are auto-generated by CLI startup and don't represent real agent work.
func (r *AgentMessagePoolRepo) CountBySessionActionable(sessionID string) (int, error) {
	var count int
	err := r.pool.QueryRow(
		`SELECT COUNT(*) FROM agent_messages WHERE session_id = ? AND content NOT LIKE '[init]%' AND content NOT LIKE '[thinking]%'`,
		sessionID,
