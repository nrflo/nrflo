...
func (workflowRunHandler) Spec() provider.ToolSpec {
	return provider.ToolSpec{
		Name:        "workflow_run",
		Description: "Start a workflow run. Returns {\"instance_id\":...}. Set ticket_id for a ticket-scoped workflow (find one with ticket_list), or omit it for a project-scoped one; workflow_list shows each definition's scope_type. A ticket-scoped definition requires ticket_id; a project-scoped one must omit it.",
