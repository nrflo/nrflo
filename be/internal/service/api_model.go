...
// enforces that "xhigh" is only used with Anthropic Opus 4.7/4.8 models.
func validateAPIReasoningEffort(provider, mappedModel, effort string) error {
	...
	if effort == "xhigh" && (provider != "anthropic" || !isXHighOpus(mappedModel)) {
		return fmt.Errorf("reasoning_effort 'xhigh' is only supported on Anthropic Opus 4.7/4.8 models")
	}
	return nil
}
