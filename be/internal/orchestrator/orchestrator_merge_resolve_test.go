...
func TestAttemptConflictResolution_NoResolverConfigured(t *testing.T) {
	...
	err := env.orch.attemptConflictResolution(context.Background(), "wfi-no-resolver", req, wt, env.pool, "merge error text", nil, "", false)
...
func TestAttemptConflictResolution_ResolvingEventBroadcast(t *testing.T) {
	...
	go func() {
		done <- env.orch.attemptConflictResolution(ctx, wfiID, req, wt, env.pool, mergeErrMsg, nil, "", false)
	}()
...
func TestAttemptConflictResolution_SpawnFails_FailedEvent(t *testing.T) {
	...
	go func() {
		done <- env.orch.attemptConflictResolution(ctx, wfiID, req, wt, env.pool, "auto-merge failed", nil, "", false)
	}()
...
func TestAttemptConflictResolution_ExtraVarsInResolvingEvent(t *testing.T) {
	...
	go func() {
		env.orch.attemptConflictResolution(ctx, wfiID, req, wt, env.pool, mergeErr, nil, "", false)
	}()
