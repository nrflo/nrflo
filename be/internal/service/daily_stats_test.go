...
func TestComputeAndStore_WithKnownData(t *testing.T) {
	...
	_, err := pool.Exec(`
		INSERT INTO workflows (id, project_id, description, created_at, updated_at)
	...
	_, err = pool.Exec(`
		INSERT INTO workflow_instances (id, project_id, ticket_id, workflow_id, status, created_at, updated_at)
