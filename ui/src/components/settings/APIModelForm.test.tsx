...
describe('APIModelForm', () => {
  ...
  describe('xhigh reasoning effort', () => {
    ...
    it('xhigh option present for anthropic with Opus 4.7 mapped model', async () => {
      ...
      expect(screen.getByText('Extra High (Opus 4.7/4.8 or Sonnet 5 only)')).toBeInTheDocument()
    })
    ...
    it('xhigh option enabled for anthropic with Opus 4.8 mapped model', async () => {
      ...
      await openEffortDropdown()
      const xhighOption = screen.getByText('Extra High (Opus 4.7/4.8 or Sonnet 5 only)').parentElement!
      expect(xhighOption).not.toHaveAttribute('aria-disabled')
    })

    it('xhigh option enabled for anthropic with Sonnet 5 mapped model', async () => {
      renderForm({
        formData: { ...emptyAPIModelForm, provider: 'anthropic', mapped_model: 'claude-sonnet-5' },
      })
      await openEffortDropdown()
      const xhighOption = screen.getByText('Extra High (Opus 4.7/4.8 or Sonnet 5 only)').parentElement!
      expect(xhighOption).not.toHaveAttribute('aria-disabled')
    ...
    it('xhigh option absent for openai provider', async () => {
      ...
      await openEffortDropdown()
      expect(screen.queryByText('Extra High (Opus 4.7/4.8 or Sonnet 5 only)')).not.toBeInTheDocument()
    })
  })
