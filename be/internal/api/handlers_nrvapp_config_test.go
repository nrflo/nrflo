...
func TestHandleListNrvappConfigFiles_NoConfigDir(t *testing.T) {
	env := newNrvappTestEnv(t)
	const pid = "proj-no-dir"
	seedNrvappProject(t, env.s.pool, pid)
	// customer_config_dir intentionally not set
	req := httptest.NewRequest(http.MethodGet, withProject("/api/v1/nrvapp/config/files", pid), nil)
	rr := httptest.NewRecorder()
	env.s.handleListNrvappConfigFiles(rr, req)
...
func TestHandleListNrvappConfigFiles_MissingProject(t *testing.T) {
	env := newNrvappTestEnv(t)
...
func TestHandleListNrvappConfigFiles_ReturnsFiles(t *testing.T) {
	cenv := newConfigTestEnv(t)

	req := httptest.NewRequest(http.MethodGet, withProject("/api/v1/nrvapp/config/files", cenv.projectID), nil)
	rr := httptest.NewRecorder()
	cenv.env.s.handleListNrvappConfigFiles(rr, req)
...
func TestHandleGetNrvappConfigFile_DBVersionTakesPrecedence(t *testing.T) {
	cenv := newConfigTestEnv(t)
	s := cenv.env.s

	// PUT a DB version
	putReq := httptest.NewRequest(http.MethodPut,
		withProject("/api/v1/nrvapp/config/content/catalog.yaml", cenv.projectID),
		strings.NewReader("sku: DB-001\n"))
	putReq.SetPathValue("file", "catalog.yaml")
	putRR := httptest.NewRecorder()
	s.handlePutNrvappConfigFile(putRR, putReq)
	...
	req := httptest.NewRequest(http.MethodGet,
		withProject("/api/v1/nrvapp/config/content/catalog.yaml", cenv.projectID), nil)
	req.SetPathValue("file", "catalog.yaml")
	rr := httptest.NewRecorder()
	s.handleGetNrvappConfigFile(rr, req)
	if rr.Code != http.StatusOK {
...
func TestHandlePutNrvappConfigFile_AutoBumpsVersion(t *testing.T) {
	cenv := newConfigTestEnv(t)
	s := cenv.env.s
	...
	put := func(content string) int {
		...
		req.SetPathValue("file", "catalog.yaml")
		rr := httptest.NewRecorder()
		s.handlePutNrvappConfigFile(rr, req)
		if rr.Code != http.StatusOK {
...
func TestHandlePutNrvappConfigFile_SchemaValidationFailure(t *testing.T) {
	cenv := newConfigTestEnv(t)
	...
	req.SetPathValue("file", "catalog.yaml")
	rr := httptest.NewRecorder()
	cenv.env.s.handlePutNrvappConfigFile(rr, req)
	if rr.Code != http.StatusBadRequest {
...
func TestHandleRollbackNrvappConfig_BroadcastsEvent(t *testing.T) {
	cenv := newConfigTestEnv(t)
	s := cenv.env.s
	pid := cenv.projectID
	...
	for _, c := range []string{"v1 content\n", "v2 content\n"} {
		req := httptest.NewRequest(http.MethodPut,
			withProject("/api/v1/nrvapp/config/content/catalog.yaml", pid),
			strings.NewReader(c))
		req.SetPathValue("file", "catalog.yaml")
		rr := httptest.NewRecorder()
		s.handlePutNrvappConfigFile(rr, req)
	...
	rollReq := httptest.NewRequest(http.MethodPost,
		withProject("/api/v1/nrvapp/config/rollback/catalog.yaml", pid),
		strings.NewReader(`{"version":1}`))
	rollReq.SetPathValue("file", "catalog.yaml")
	rollRR := httptest.NewRecorder()
	s.handleRollbackNrvappConfig(rollRR, rollReq)
	...
	ev := cenv.env.rec.waitEvent(t, ws.EventNrvappConfigUpdated)
...
func TestHandleGetNrvappConfigFile_PathTraversal(t *testing.T) {
	cenv := newConfigTestEnv(t)
	s := cenv.env.s
	pid := cenv.projectID
	...
	for _, bad := range badPaths {
		req := httptest.NewRequest(http.MethodGet,
			withProject("/api/v1/nrvapp/config/content/"+bad, pid), nil)
...
func TestHandlePutNrvappConfigFile_PathTraversal(t *testing.T) {
	cenv := newConfigTestEnv(t)
	s := cenv.env.s
	pid := cenv.projectID

	req := httptest.NewRequest(http.MethodPut,
		withProject("/api/v1/nrvapp/config/content/../escape", pid),
