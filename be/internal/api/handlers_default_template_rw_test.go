...
func TestHandleDefaultTemplate_FullCRUDFlow(t *testing.T) {
	...
	// 1. List — 9 pre-seeded readonly.
	listRR := httptest.NewRecorder()
	s.handleListDefaultTemplates(listRR, httptest.NewRequest(http.MethodGet, "/api/v1/default-templates", nil))
	if got := decodeDefaultTemplateList(t, listRR); len(got) != 9 {
		t.Fatalf("initial list len = %d, want 9", len(got))
	}
	...
	if got := decodeDefaultTemplateList(t, listRR2); len(got) != 10 {
		t.Fatalf("after create list len = %d, want 10", len(got))
	}
	...
	if got := decodeDefaultTemplateList(t, listRR3); len(got) != 9 {
		t.Errorf("after delete list len = %d, want 9", len(got))
	}
}
