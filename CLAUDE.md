...
16. **Low-context relaunch**: When an agent's context drops below threshold (default ~25% remaining, configurable per agent via `restart_threshold` in agent_definitions), the spawner kills the agent and saves context via one of two methods controlled by the `context_save_via_agent` global setting (default false). When **enabled**: spawns a `context-saver` system agent (haiku model) to summarize the killed agent's message history; works for all CLI types. When **disabled** (default): resumes the same Claude session with a save prompt so the agent writes its own summary; only works for Claude CLI, other CLIs skip context save. Both methods write progress under the `to_resume` findings key. A fresh agent is then spawned with `${PREVIOUS_DATA}` populated from that key. Old sessions get `status='continued'` and are excluded from agent history.
...
37. **Interactive CLI backend**: When the per-project `interactive_cli_mode` setting is enabled, the spawner uses `cliInteractiveBackend` instead of `cliBackend` for CLI agents whose adapter returns `SupportsInteractive() == true`. Today only Claude and Codex qualify; Opencode returns `false` (no hook system → no structured visibility) and always falls back to `cliBackend` regardless of the toggle. The agent process is spawned inside a PTY (no batch flags). Prompt delivery asymmetry: Claude receives the rendered body via PTY stdin Write after a ~250ms readiness delay; Codex receives it as the final argv positional (the codex TUI input box has a wrapping bug at `tui/src/wrapping.rs:52` that panics on multi-KB pasted bodies). Suffix delivery asymmetry: Claude uses `--append-system-prompt-file`, Codex prepends the suffix to the prompt body in memory. Hook telemetry delivery split: Claude receives hooks via `--settings` JSON; **Codex receives hooks via repeated `-c hooks.<event>=…` CLI flags** (codex's TUI ignores `<CODEX_HOME>/config.toml` hook tables in PTY contexts; only the `-c` session-layer override path actually fires hooks). The Codex per-session `CODEX_HOME` profile dir carries auth, model/personality settings, `[features] codex_hooks = true`, and a `[projects."<workDir>"] trust_level = "trusted"` entry — but no hook tables. PTY ferry auto-answers DSR/DA/kitty/OSC terminal-capability queries so codex's TUI can initialize in our creack/pty PTY. Take-control in this mode is a **viewer-attach** — the agent is not killed; `agent.viewer_attached` is broadcast and the viewer connects via the standard `/api/v1/pty/{session_id}` endpoint (which accepts `status=running` when a PTY session already exists). Viewer disconnect does not trigger `exit-interactive`. System agents (conflict-resolver, context-saver-CLI) inherit the same backend selector. `spawner.Config.PTYManager` carries the PTY manager from the orchestrator (`Orchestrator.PTYManager`, wired in `server.go`). Codex interactive telemetry is wired through the rollout JSONL referenced by every hook payload's `transcript_path`. The `agent.record_event` handler runs two extractors: (1) on every fire, scans for the latest `token_count` event and forwards `100 - 100*last_token_usage.input_tokens/model_context_window` through the standard `agent.context_update` path (low-context restart works the same as for Claude); (2) on `Stop`, flushes new `event_msg/agent_message` records since the per-session byte offset (in-memory, not persisted across restarts) into `agent_messages` rows so the model's spoken text is visible in the UI — codex hooks expose tool I/O and user input but never model output, which lives in the JSONL only. Reasoning blocks are NOT extracted: codex 0.125 emits only encrypted reasoning content (`encrypted_content` blob, no plaintext or summary). See [be/internal/spawner/CLAUDE.md](be/internal/spawner/CLAUDE.md) for full backend selection rules, PTY lifecycle, and output ferry behavior.
