...
func (workflowContinueHandler) Spec() provider.ToolSpec {
	return provider.ToolSpec{
		...
		Description: "Resume a workflow instance whose status is exactly `waiting` — parked by a pause_after layer — from its pause point. Any other status is rejected, including the plan-boundary statuses (planning/waiting_input/waiting_approval): a plan-driven run is resumed by approve_plan, not this tool.",
