...
Three call sites compose instead of passing raw narrative text: the `${previous_data}` relaunch injection (`spawner/template_findings_prev.go`), `GET /api/v1/sessions/{id}/handoff-digest` (`api/handlers_handoff_digest.go`), and the `agent.handoff_digest` WS broadcast (`refinery/session_sidecar.go`). All three fall back to the raw narrative/digest content when `Compose` returns `""`. A fourth consumer, the spawner's stepwise resume body (`spawner/template_stepwise_resume.go`), calls `NarrativeSection` directly instead of `Compose` — its document is completed-step evidence, not a generic Verified State.
