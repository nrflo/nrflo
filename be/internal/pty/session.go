...
func NewSession(sessionID, workDir string, env []string, command string, args []string) (*Session, error) {
	...
	// starts at 0x0, and TUIs that probe TIOCGWINSZ on startup immediately
	// exit with code 1 before any client attaches to send a real resize.
	// 80x24 matches POSIX defaults and is harmless for adapters that ignore it.
