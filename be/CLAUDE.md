...
HTTP routes use SCS cookie-based sessions (`nrflo_session`). Handler chain: `cors → requestID → projectMiddleware → LoadAndSave → mux`. Routes are `protected` (requireAuth), `admin` (requireAdmin), or public (login only). `requireAuth` also accepts `Authorization: Bearer <agent_token>` (spawner-minted `spawn_token`, valid while session is `running`/`user_interactive`; bearer requests do not populate user context so `requireAdmin` always 403s them). Login rate limiter: 5 attempts per 5 min per IP+email, returns 429 with `Retry-After`. See [be/internal/auth/CLAUDE.md](internal/auth/CLAUDE.md) and [be/internal/api/CLAUDE.md](internal/api/CLAUDE.md).

The Unix socket uses JSON-RPC line-delimited protocol. Supported methods: `findings.*`, `project_findings.*`, `agent.fail/continue/callback/context_update`, `workflow.skip`, `ws.broadcast`.
