...
func newTestRepos(t *testing.T, clk clock.Clock) (*repo.NrvappDispatchRepo, *repo.NrvappReviewRepo, string) {
	t.Helper()
	...
	return repo.NewNrvappDispatchRepo(database, clk),
		repo.NewNrvappReviewRepo(database, clk),
		"proj-1"
}
...
func TestManifestHandler_HappyPath_NoReview(t *testing.T) {
	...
	// EventNrvappDispatchCompleted once; no review event.
	events := hub.Events()
...
func TestManifestHandler_HappyPath_WithReview(t *testing.T) {
	...
	if n := countEvents(events, ws.EventNrvappReviewCreated); n != 1 {
		t.Errorf("review events = %d, want 1", n)
	}
	for _, e := range events {
		if e.Type == ws.EventNrvappReviewCreated {
