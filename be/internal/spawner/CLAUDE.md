...
- **`cli_interactive`** — spawns the CLI inside a PTY without batch flags. Default for claude/opencode/gemini. **Codex is the exception**: codex/cli_interactive is routed to the codex app-server backend below (not the PTY).
...
- `BumpsOnPTYBytes()` — opt-in heartbeat via PTY bytes. Claude (hooks), Opencode (SSE), and Gemini (JSONL tailer) return false (structured channels drive the heartbeat). The codex PTY adapter is unused — codex runs on the app-server backend.

The above bullets describe the **PTY** path (claude/opencode/gemini). **Codex does not use it** — see [Codex app-server backend](#codex-app-server-backend). `writeCodexProfileForSession` (`cli_adapter_codex_profile.go`) writes the per-session `CODEX_HOME` (auth + a `[projects."<path>"] trust_level="trusted"` entry — codex 0.133 reads workdir trust from `CODEX_HOME/config.toml`, and without it the session blocks on a directory-trust dialog); it is now called by the app-server backend's `Start`.
...
- **System agent** (`context_save.go`): spawns a fresh `context-saver` haiku agent that reads message history and writes `to_resume` findings. Used for opencode, api-mode, codex (app-server backend), and when `Config.ContextSaveViaAgent == true`.

Selected by `shouldUseAgentSave(proc)` off `proc.backend.SupportsResume()`: claude → resume (with agent-path fallback); codex/opencode/api → agent. Script-mode agents are exempt (`TracksContext()=false`).
