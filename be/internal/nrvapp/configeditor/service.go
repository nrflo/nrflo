...
func NewService(configDir string, manifest *config.Manifest, r *repo.NrvappConfigVersionRepo, clk clock.Clock) *Service {
...
func (s *Service) History(projectID, file string) ([]*model.NrvappConfigVersion, error) {
	if err := s.validateRelPath(file); err != nil {
		return nil, err
	}
