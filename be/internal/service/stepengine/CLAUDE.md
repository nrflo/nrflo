...
`New(pool, clk, checks CheckRunner) *Engine` — `checks` is the injectable command runner (`RunChecks`, same return shape as `spawner.runValidationCommands`); nil skips checks. `complete_step` (`apirun/tools_builtin/complete_step.go`) is the only call site that supplies a real one, via `apirun.StepSession.RunStepChecks` → the spawner's shared shell executor; every other `stepengine.New` call site (`template_stepwise.go`, `template_stepwise_resume.go`) still passes nil because it never advances. `Engine.Snapshot`, `Engine.State`, `Engine.ValidateEvidence`, `Engine.Advance`, `Engine.CompletedEvidence` (structured per-completed-step evidence — snapshot-declared keys/values/resolved-paths, no prompt prose, consumed by the spawner's stepwise resume body), and the pure `ShouldRotate` are the package's exported surface — see `stepengine.go`/`snapshot.go`/`evidence.go`/`advance.go`/`rotate.go`/`evidence_digest.go`.
