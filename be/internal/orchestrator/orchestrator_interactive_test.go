...
func TestNewAgentDefinitionService_IsAvailable(t *testing.T) {
	env := newTestEnv(t)
	svc := service.NewAgentDefinitionService(env.pool, clock.Real(), service.NewCLIModelService(env.pool, clock.Real()), service.NewAPIModelService(env.pool, clock.Real()), nil)
	if svc == nil {
