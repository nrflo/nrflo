"""S35 — Custom cli_models row resolves to a real CLI binary (opencode).

Mirrors claude/s35: registers a brand-new cli_models row with
`cli_type=opencode` and a known opencode model name as `mapped_model`.
The spawner must resolve the agent_def through `spawner.cliForModel`
to the opencode binary and complete normally; `agent_sessions.model_id`
is stored as `<cli_type>:<id>` (spawner.go:898).
"""
...
def run(ctx: Ctx) -> Result:
    ...
    model_id = next_id(ctx, "cm-opencode-mini")
    ctx.client.create_cli_model(
        id=model_id,
        cli_type="opencode",
        display_name=f"Custom Opencode Mini ({model_id})",
        mapped_model="openai/gpt-5.4-mini",
    ...
    return ("S35 custom cli_model", "PASS",
            f"resolved {sess_model} → opencode/gpt-5.4-mini, session={sess['id'][:8]}")
