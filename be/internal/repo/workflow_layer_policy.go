...
func (r *WorkflowLayerPolicyRepo) Upsert(policy *model.WorkflowLayerPolicy) error {
	...
	_, err := r.db.Exec(`
		INSERT INTO workflow_layer_policies (project_id, workflow_id, layer, pass_policy, pause_after, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?)
...
func (r *WorkflowLayerPolicyRepo) ListByWorkflow(projectID, workflowID string) ([]*model.WorkflowLayerPolicy, error) {
	rows, err := r.db.Query(`
		SELECT project_id, workflow_id, layer, pass_policy, pause_after, created_at, updated_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&p.ProjectID, &p.WorkflowID, &p.Layer, &p.PassPolicy, &p.PauseAfter, &createdAt, &updatedAt); err != nil {
			return nil, err
		}
...
func (r *WorkflowLayerPolicyRepo) GetLayerPauseAfter(projectID, workflowID string) (map[int]bool, error) {
	policies, err := r.ListByWorkflow(projectID, workflowID)
	if err != nil {
		return nil, err
	}
	result := make(map[int]bool, len(policies))
	for _, p := range policies {
		result[p.Layer] = p.PauseAfter
	}
	return result, nil
}
