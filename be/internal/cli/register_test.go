...
func TestRegisterServerCommands(t *testing.T) {
	...
	unexpectedCommands := []string{"agent", "findings", "tickets", "deps", "observer"}
...
func TestRegisterCLICommands(t *testing.T) {
	...
	// Expected: agent, findings, tickets, deps, skip, observer, version
	expectedCommands := map[string]bool{
		"agent":    true,
		"findings": true,
		"tickets":  true,
	...
	// Verify exact count: agent + findings + tickets + deps + skip + observer + version = 7
	if len(commands) != 7 {
		t.Errorf("RegisterCLICommands: got %d commands, want 7. Commands: %v", len(commands), commands)
	}
}
...
func TestAgentSubcommands(t *testing.T) {
	...
	expectedSubcommands := []string{"fail", "finished", "continue", "callback", "context-update", "record-event", "statusline", "chain-next-instructions", "chain-next-ticket", "consult"}
	...
	// Verify exact count: 11 subcommands (artifact + consult added)
	if len(actualSubcommands) != 11 {
		t.Errorf("agentCmd has %d subcommands, want 11. Subcommands: %v", len(actualSubcommands), actualSubcommands)
	}
}
