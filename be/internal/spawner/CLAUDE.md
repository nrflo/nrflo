...
- **`cli_interactive`** — spawns the CLI inside a PTY without batch flags. Default for claude. **Codex is the exception**: codex/cli_interactive is routed to the codex app-server backend below (not the PTY). For **both Claude and codex**, `prepareSpawn`'s cli tail attaches the nrflo tool registry to `proc` via `attachNrfloToolRegistry` (`mcp_tools.go`): it honors the agent definition's `tools` CSV (empty → `"*"`, the full set, for backward compatibility) and calls `buildAPIRegistry(…, toolsCSV, forceBaseline=true)` so the `agent_*` lifecycle tools AND `findings_add` are always present even under a restrictive CSV. Tools are served over `tools.list`/`tools.call` by the `nrflo_server agent mcp` bridge, with `read_document` swapped to the path-returning variant. The agent then calls the nrflo tools over MCP instead of the nrflo CLI. The transport differs per adapter: Claude gets `--mcp-config`/`--strict-mcp-config`/`--allowedTools mcp__nrflo__*` (`configureClaudeMCPTools`; native coding tools untouched, no `--tools`); codex gets a `[mcp_servers.nrflo]` + `[mcp_servers.nrflo.env]` table appended to its `CODEX_HOME/config.toml` (`appendCodexMCPServer`), with the bridge env embedded because codex does not forward parent env to MCP subprocesses.
