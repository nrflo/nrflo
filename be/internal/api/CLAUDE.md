...
`POST /api/v1/console/chats` (`projectAdmin`, body `{engine, model}`) mints a `kind='console_chat'` session and starts its `console.ChatService`-owned engine; `GET /api/v1/console/chats` (`projectAdmin`) lists the project's chat rows with a `live` flag from `ChatService.Live`. Every other route is path-scoped by `{sid}` (never `{id}` — see `server_routes_sessions.go`'s doc comment) and `protected`, authorized in-handler by the same admin/service-principal/own-bearer predicate as console-session close: `GET .../{sid}` (row fields plus `ChatService.Snapshot`'s turn/work_dir/pending_approvals, for restoring a reloaded page), `POST .../{sid}/messages` (409 on a turn already in flight), `POST .../{sid}/approvals/{aid}` (body `{decision: allow|deny}`), `POST .../{sid}/close`, `GET .../{sid}/messages` (history). Live deltas/turn/approval events stream over the WS session-subscription channel, not these routes — see [ws/CLAUDE.md](../ws/CLAUDE.md). Every approval is settled by exactly one `console_chat.approval_resolved` push, whether by human reply, timeout, or engine stop.
