	...
	for composer := 1; composer <= 8; composer++ {
		for _, suggestionMatches := range []int{0, 3, 12} {
			for _, approvalCount := range []int{0, 1} {
				chrome := chromeRows(composer, suggestionMatches, approvalCount)
...
func TestChromeRows_ExtremelySmallTerminal(t *testing.T) {
	chrome := chromeRows(8, 12, 1) // composer maxed out + full suggestion box + approval
...
func TestChromeRows_ComponentsAdditive(t *testing.T) {
	base := chromeRows(1, 0, 0)
	withSuggestions := chromeRows(1, 3, 0)
	withApproval := chromeRows(1, 0, 1)
	withBoth := chromeRows(1, 3, 1)
