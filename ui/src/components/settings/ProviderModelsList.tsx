...
export function ProviderModelsList({ provider }: Props) {
  ...
  const { data: allModels = [], isLoading, error } = useCLIModels()
  const models = allModels.filter((m) => m.cli_type === provider)
  ...
  const handleStartCreate = () => {
    setIsCreating(true)
    setEditingId(null)
    setFormData({ ...emptyCLIModelForm, cli_type: provider })
  }
